...
export class DomindsCodeBlock extends HTMLElement {
  ...
  private render() {
    ...
    const copyCodeText = '复制代码 / Copy code';
      ...
      this.innerHTML = `
        ...
        <div class="code-block-wrapper">
          <div class="code-header">
            ...
            <button class="copy-btn" title="${escapeHtmlText(copyCodeText)}" aria-label="${escapeHtmlText(copyCodeText)}"><span class="icon-mask" aria-hidden="true"></span></button>
          </div>
  ...
  private setupCopyButton(): void {
    ...
    button.addEventListener('click', () => {
      void this.copyCodeToClipboard(button);
    });
  }

  private async copyCodeToClipboard(button: HTMLButtonElement): Promise<void> {
    try {
      await copyTextOrShowManualCopy(this._code, button);
    } catch (error: unknown) {
