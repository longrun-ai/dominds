...
export class DoneDialogList extends HTMLElement {
  ...
  public applySnapshot(dialogs: ApiRootDialogResponse[]): void {
    ...
    for (const rootId of rootIds) {
      if (!this.visibleSubdialogCountByRoot.has(rootId)) {
        this.visibleSubdialogCountByRoot.set(rootId, DoneDialogList.SHOW_MORE_STEP);
      }
    }
      ...
      if (!isRoot) {
        ...
        if (subIndex >= 0) {
          const currentSubLimit =
            this.visibleSubdialogCountByRoot.get(rootId) ?? DoneDialogList.SHOW_MORE_STEP;
  ...
  private showMoreSubdialogs(rootId: string): void {
    const current = this.visibleSubdialogCountByRoot.get(rootId) ?? DoneDialogList.SHOW_MORE_STEP;
    this.visibleSubdialogCountByRoot.set(rootId, current + DoneDialogList.SHOW_MORE_STEP);
    this.applySnapshot(this.snapshotDialogs);
  }
