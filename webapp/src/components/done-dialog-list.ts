...
export class DoneDialogList extends HTMLElement {
  ...
  public updateDialogEntry(
    ...
    if (patch.lastModified !== undefined) {
      ...
      if (targetSelf === rootId) {
        ...
        const mainDialog = this.findDialogByIds(rootId, rootId, true);
        if (mainDialog) {
  ...
  public applySnapshot(dialogs: ApiMainDialogResponse[]): void {
    ...
    for (const taskPath of taskPaths) {
      if (!this.visibleRootCountByTask.has(taskPath)) {
        this.visibleRootCountByTask.set(taskPath, DoneDialogList.SHOW_MORE_STEP);
      }
    }
    ...
    for (const rootId of rootIds) {
      if (!this.visibleSideDialogCountByRoot.has(rootId)) {
        this.visibleSideDialogCountByRoot.set(rootId, DoneDialogList.SHOW_MORE_STEP);
      }
    }
  ...
  private reconcileSideDialogRows(rootChildren: HTMLElement, rootGroup: RootGroup): void {
    ...
    const visibleSideDialogCount =
      this.visibleSideDialogCountByRoot.get(rootGroup.rootId) ?? DoneDialogList.SHOW_MORE_STEP;
    ...
    for (const sideDialog of visibleSideDialogs) {
      ...
      const next = this.createElementFromHtml(this.renderDialogRow(sideDialog));
  ...
  private reconcileRootNodes(
    ...
    const visibleRootCount =
      this.visibleRootCountByTask.get(group.taskDocPath) ?? DoneDialogList.SHOW_MORE_STEP;
  ...
  private renderDialogRow(dialog: ApiMainDialogResponse): string {
    const t = getUiStrings(this.props.uiLanguage);
    const isSelected = this.isSelectedDialog(dialog, this.selectionState);
    const updatedAt = dialog.lastModified || '';
  ...
  private handleClick = (event: Event): void => {
    ...
    if (actionEl) {
      ...
      if (action === 'root-create-dialog') {
        const rootId = actionEl.getAttribute('data-root-id');
        if (rootId) {
          const mainDialog = this.findDialogInSnapshot(rootId, rootId);
          if (mainDialog) {
      ...
      if (action === 'root-delete') {
        const rootId = actionEl.getAttribute('data-root-id');
        if (rootId) {
          this.emitDeleteAction({ kind: 'root', rootId, fromStatus: 'completed' });
        }
        return;
      }
      ...
      if (!isRoot) {
        ...
        if (subIndex >= 0) {
          const currentSubLimit =
            this.visibleSideDialogCountByRoot.get(rootId) ?? DoneDialogList.SHOW_MORE_STEP;
  ...
  private showMoreRoots(taskDocPath: string): void {
    const current = this.visibleRootCountByTask.get(taskDocPath) ?? DoneDialogList.SHOW_MORE_STEP;
    this.visibleRootCountByTask.set(taskDocPath, current + DoneDialogList.SHOW_MORE_STEP);
    this.applySnapshot(this.snapshotDialogs);
  }

  private showMoreSideDialogs(rootId: string): void {
    const current = this.visibleSideDialogCountByRoot.get(rootId) ?? DoneDialogList.SHOW_MORE_STEP;
    this.visibleSideDialogCountByRoot.set(rootId, current + DoneDialogList.SHOW_MORE_STEP);
    this.applySnapshot(this.snapshotDialogs);
  }
