...
这条边界是架构约束，不是命名偏好：不要把不同业务 continuation 提炼成统一的
“edge vs level”、“durable fingerprint”、“统一清理”、“综合 can-drive 判定”或类似技术抽象。
这些抽象会抹掉真实业务事实，把多个 continuation 的局部时序判断塞进一个大 gate，最终变成
`source + reason + trigger + displayState + executionMarker + latest` 互相猜测的意大利面。

正确方向是：

- 每一种业务 continuation 都有自己的本地 claim 闭包。
- 这个闭包只读取本业务需要的持久事实，只消费/失效本业务自己的 continuation 元数据。
- 调度层只负责叫醒和路由，不替任何业务 continuation 做“综合价值判断”。
- 没有某个具体业务 continuation 明确 claim 成功，就没有业务依据；没有业务依据，就没有业务价值，也就不能自动 drive。
- 不允许用通用 fingerprint、历史回扫、模糊匹配或多业务统一 cleanup 来替代本地 claim。

如果一个修复方案需要新增“所有 continuation 共用的清理/去重/是否继续”判断，先判定为设计异味。
除非能证明它只是路由层机械调用各自 handler，而不是汇总业务事实，否则不要落地。
...
- `requested_work_replied(batchId)`：读取 `active-callees.json` / batch 状态。batch 仍 resolved 且未被 caller gen turn 接受，则 claim，并确保同 batch 的 `result_arrival` trigger 存在以进入现有 gen-start 消费管线；gen start 接受该 trigger 后移除 active-callees batch，表示该回复事实已归属本轮 LLM context。batch 已不存在就是已消费 / stale，`debug` 丢弃，不再 revive；batch 仍 open 却收到 resolved revive 是状态冲突，loud fail。
...
- `entitlement` / `revive`：业务上不是某个 dialog 获得“恢复权”，而是“有新事实到达，caller 应处理”。目标实现以业务 continuation 表达这件事，例如 `requested_work_replied(batchId)` 或 `deliver_tellask_reply(targetCallId)`；若底层仍临时保留 entitlement token，只能作为迁移期防重入/防误续推的内部令牌，并必须携带同一个业务 identity。
- `wait-group`：业务上不是 caller 正在等待一组 callee，而是“同一次派发批次的回贴结果要成组收口”。因此设计用 dispatch batch / 派发批次描述，不再新增 `waitGroupId` 这类等待语义字段名。
- `open generation recovery`：业务上不是恢复 proceeding 状态，而是“上一次 generation 确认未闭合，所以继续同一轮”。已闭合 generation 只能从状态快照重新投影状态。
...
- 不用“回扫历史聊天记录”来推导 open generation、dispatch batch completion、reply delivery 是否完成、active reply obligation、next-step 来源等运行判定。
- 历史事件日志只保留为审计、调试、人类追查、上下文展示/组装，以及 dialog fork 复制输入；它不是常态业务判定的数据源。
- Dominds 永远不制作“历史数据一次性迁移 / repair 工具”作为软件功能。对话历史是低价值数据；正常处理流程是依赖工作区文件、差遣牒进展记录和必要的人类判断，开新对话继续推进。遇到旧对话运行态缺失或损坏时，系统应 loud diagnostic + 停止不安全路径，而不是提供历史迁移/修补软件去抢救旧对话。最接近“复用历史”的能力是 dialog fork：它可以扫描并复制对话历史，但只到复制新分支为止，不承担修复旧运行态的职责。
...
- next-step trigger 元信息：下一步动作的具体来源、来源 course/genseq/callId、是否一次性消费。
...
type NextStepTrigger =
  ...
  | (NextStepTriggerBase & { kind: 'root_drive_wake'; reason: string; course: number })
  | ({
      kind: 'followup';
...
`nextStep.triggers.length > 0` 表示“有未消费的新事实”，不等价于“此刻允许启动 drive”。backend loop / run-control 必须同时读取 user wait 等先决等待事实；只有先决等待为空时，未消费 trigger 才能进入 runnable drive。
...
`active-callees` is an observability fact and, for mainline dialogs only, a Diligence Push input. It must not decide whether the caller dialog can keep working. Its business role is narrower: for `requested_work_replied(batchId)`, it is the consumption ledger that says whether that specific reply batch is still waiting to be accepted by a caller generation.
...
1. 旧 backend loop 会驱动 `globalDialogRegistry.needsDrive=true` 的主线对话。
...
这解释了“多数场景符合，但该现场不符合”：多数场景靠旧误投影恰好停住；该现场被 queued/open generation recovery 绕过了旧误投影。
...
- `recovery/open-generation-recovery.ts` 和 `recovery/reply-delivery-recovery.ts`。
...
2. `recoverOpenGenerationAfterRestart()`
3. `recoverPendingReplyDeliveryAfterRestart()`
4. `runBackendDriver()`

当前规则：

- `isRecoverableGeneratingLatest(latest)` 对 `generating=true` 且没有强 interrupted/dead marker 的 dialog 返回 true。
- 旧 restart reconciliation 把它设为 `needsDrive=true`、`displayState=proceeding`。
- 旧主线 open generation recovery 将主线对话标记为 needsDrive。
...
最后一种不应继续存在。pending tellask 是后台事实，不是“待驱动队列”的理由。重构后 `needsDrive` boolean 投影应删除，真实调度来源必须是 `NextStepTriggerState.triggers.length > 0` 或 open `generationRunState`，且每个 trigger 必须是显式 union variant：`user_input`、`queued_prompt`、`mainline_diligence`、`result_arrival`、`open_generation_recovery`、`reply_delivery_recovery`。
...
#### 5.2 closed / missing generation state 不进入 open generation recovery

`generating=true` 是投影，不是恢复事实。当前代码已把 open-generation recovery 的判定收敛到 `getRecoverableGenerationRunState(latest)`：只有 `generationRunState.kind === 'open'` 且 interruption marker 允许自动恢复时，才写入 `open_generation_recovery` trigger 并进入 open generation recovery。
...
结构化日志字段至少包含 `rootId`、`selfId`、`course`、`genseq`、`latestGenerating`、`nextStepTriggerCount`、`decision.kind`、`decision.reason`。
...
- 清 `generating=false`。
- 清不必要 queued state；除非存在 queued prompt、pending course start 转换出的 `queued_prompt` trigger、或 reply recovery trigger，否则不写 next-step trigger。
...
第一阶段时 `recoverPendingReplyDeliveryAfterRestart()` 通过扫描历史 call-without-result 推导未交付 reply；第二阶段已改为读取 `latest.replyDelivery`。目标态继续要求 reply delivery 元信息记录完整 `deliveryStatus`，重启后直接读取状态快照恢复未完成交付。
...
#### 5.5 restart 后 next-step 的收敛规则

closed-generation projection 清掉 stale `generating` 后，要同步处理 `nextStep.triggers`：

- 如果存在 `pendingCourseStartPrompt`：转成 `queued_prompt` trigger；display 为对应 stopped/resumable 状态。
- 如果存在 pending reply delivery：由 `recoverPendingReplyDeliveryAfterRestart()` 负责产生/保留 `reply_delivery_recovery` trigger。
- 如果只是 pending tellask / pending sideDialogs：不写 next-step trigger，display 投影为 `idle_waiting_user`，pending tellask 通过 reminder/panel 表达。
- 如果状态快照缺少必要元信息：转移到 `malformed/` 并发 warning / structured diagnostic，不要把 next-step trigger 当兜底保活，也不要回扫历史补猜。
...
- schedule 等待方 drive 只负责叫醒；真正是否继续，由 `requested_work_replied(batchId)` continuation 在轮到时重新读取 `active-callees` batch / user wait 等最新业务事实后 claim；`result_arrival` trigger 只是可补建的 gen-start handoff 材料，不是业务授权。
...
- restart reconciliation 在 patch latest 时同时处理 `generating`、`nextStep`、`displayState`、`executionMarker`，避免留下 stale queued 状态。
...
- backend loop drive 前后更新 `nextStep` / drive wake 时，不得因 pending tellask 保持 queued；调度 runnable drive 时必须同时检查 `userWait.kind === 'none'`。

### `main/recovery/open-generation-recovery.ts`
...
- 不再在 open generation recovery 中抢在 reply recovery 前误驱动 closed generation。

### `main/recovery/reply-delivery-recovery.ts`

- 旧实现扫描 `replyTellask*` call-without-result；目标态改为读取 reply delivery 元信息。扫描历史不作为 runtime recovery，也不作为迁移/repair 工具入口。
...
- crash recovery 从 `active-callees.json` 读取每个 batch 的 pending/resolved/final 状态；不从 durable events 重建运行态，也不提供历史迁移/repair 工具。
...
1. caller dialog 单个 `tellask` 后不 immediate post-tool continue。
   ...
   - 期望：不产生 `result_arrival` trigger；pending tellask dispatch ack 不写入 `nextStep` / `root_drive_wake`。
...
1. closed single pending tellask generation + stale `generating=true`。
   - 构造：状态快照有 `generationRunState.kind=closed`、无 `followup` trigger、pending dispatch batch，latest 仍有 stale `generating=true` 或旧 `needsDrive=true` 投影。
...
后台 callee dispatch 已收敛到 `active-callees.json`，旧 `pending-sideDialogs.json` 已从运行源里移除。当前运行事实只来自状态快照和显式 pending records，常态业务判定不再回扫 course JSONL 补猜运行事实；course JSONL 只保留为 transcript/context/审计/调试/人类追查/dialog fork 复制输入。

运行与恢复的核心状态已全部显式化：

- `generationRunState` 已写入 `latest.yaml`，generation start 标记 `open`，generation finish 标记 `closed`，restart open-generation recovery 只认 `generationRunState.kind === 'open'`。
- `NextStepTriggerState` 已落地，`queued_prompt`、`root_drive_wake`、`followup`、`mainline_diligence`、`result_arrival`、`open_generation_recovery`、`reply_delivery_recovery` 都是 durable trigger；generation start 会消费 accepted trigger，不保留 consumed tombstone。
...
- Root backend wake 只通过显式 next-step API 表达：`root_drive_wake` trigger + registry wake signal；不再存在 `backend_queue` bridge 或 `needsDrive` runtime/API 调用面。
- `globalDialogRegistry` 只承载 live registry 与 wake signal，`wakeDrive` / `clearDriveWake` / `isDriveWakeQueued` 是唯一公开语义。
- backend loop 只扫描 live root dialog 本身 + root-local `wake-cues.json` 中的支线子集；支线只在 `latest.nextStep.triggers`、open `generationRunState`、pending/delivered-but-tool-result-pending `replyDelivery` 等 durable work 存在时进入 wake cue 子集。
...
- pending tellask 不再投影为 `waiting_for_sideDialogs`；前端以后台被诉请者数量 / FBR 被诉请者数量表达可观测状态。
- `dlg_background_callee_summary_evt` 已落地，active callee 增加、resolve、registered tellask 替换旧 owner 等路径会广播后台被诉请者数量，前端 dialog list 会及时更新电话听筒 / FBR badge。
- Diligence Push 已统一通过 `evaluateDiligenceAutoContinueGate()` 判断：active callee / Q4H / stop/interrupted 等边界会阻止鞭策 prompt；若 root 仍有 pending active callee，不消耗预算、不插入鞭策语、不维持主线空转。

重启和异常边界也已收口：

- `generating=true` 但缺少 `generationRunState` 的 dialog 会进入 `malformed/`，不再静默停成 `server_restart` 或回扫历史补猜。
- `reply-delivery-recovery` restart recovery 已改为读取 `latest.replyDelivery`，不再扫描当前 course events 查找 call-without-result。
- duplicate pending `replyDelivery` 采用保活路径：成功交付后同步 delivered；遇到 stale pending delivery 且新 reply call 对应当前有效 reply obligation 时，记录 loud warn 并替换 pending delivery，避免可恢复对话被 hard stop。
- runtime reason / error 已收敛到 result-arrival / dispatch-batch 语义；旧 `tellask-revive-context-refactor` 历史设计文档已删除。
...
3. 下一刀：迁移 `result_arrival` / caller revive，用 `requested_work_replied(batchId)` continuation 替换 revive entitlement；direct schedule 和 backend watch 都只叫醒，claim 时读取 `active-callees` batch 最新事实，并确保同 batch 的 gen-start handoff trigger 可用。
...
13. 调整 open generation recovery，只恢复真正 open generation。
...
- 每个业务 continuation 的即时判断逻辑必须本地化在自己的 handler / closure 内；不要新增跨 continuation 的综合判定、综合清理或通用去重层。
- 调度、backend wake、wake cue、`source/reason`、迁移期 entitlement token 只能是路由或叫醒机制，不能被当成业务授权。
- 没有业务依据，没有业务价值；无 prompt 自动 drive 必须来自某个具体 continuation 的成功 claim，而不是“当前状态看起来还能跑”。
- 不允许用 edge/level、fingerprint、历史 transcript 扫描或模糊匹配替代业务事实 claim。
...
- 已有 `gen_finish_record` 的 generation 不应被 open generation recovery 重放。
