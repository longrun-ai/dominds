...
本计划记录一次围绕 `tellask` 主线续推行为的语义收敛，并把它扩展到后台进程崩溃/重启后的自动恢复逻辑。
...
- 同一 rtws 的 `latest.yaml` 后续落在 `generating: false`、`needsDrive: true`、`displayState: blocked/waiting_for_sideDialogs`。这说明当前系统把“后台诉请仍 pending”误投影成了“主线等待支线”的运行状态；大多数场景看起来会停，靠的是这个旧误投影，而不是最终原则。

这三个事实共同说明：最终原则不是要修复一个单点 if，而是要把 `tellask` background fact、下一步动作判定、Diligence Push、restart recovery 四套语义拆清楚。这里的 dialog 不是只指主线；主线和支线都可能作为 asker 发起后台诉请，也都可能通过 `tellaskBack` / ask-back 路径成为 callee 并承担回贴义务。
...
- pending tellask 不表示发起方 dialog 必须停下；发起方可以是主线，也可以是支线。
- pending tellask 不要求发起方 dialog 等待回贴。
- pending tellask 不等价于 `waiting_for_sideDialogs` 显示/运行状态。
- 发起方 dialog 可以继续，也可以自然 idle；这由智能体后续动作、用户输入、queued prompt、Diligence Push 等通用机制决定。
- 被诉请方 dialog 的 active reply obligation 是另一种事实：它表示“当前 dialog 最终必须交付回贴”，但不妨碍该 dialog 自己发起下游 pending tellask。

### 角色相对性

同一个 dialog 可能同时处在多个相对角色中，不能用“主线/支线”静态划分控制流：

- 作为 asker：它发起 `tellask` / `tellaskSessionless` / `tellaskBack`。它发出的 pending tellask 对它自己是后台事实，不决定“现在必须停下”。
...
因此规则必须按“当前 dialog 相对于某条诉请/回贴链路的角色”判定，而不能写成“主线停、支线停”或“所有 sideDialogs 都等待”。主线被 `tellaskBack` 时同样是 callee；支线发起下游 `tellask` 时同样是 asker。
...
Diligence Push 是例外：它只作用于主线对话，因为它是编排保活机制。支线不被运行时鞭策；支线如果出现非本意的直接回复并停止驱动，应通过 direct-fallback 把事实交回 caller，由 caller 智能判断是续推原诉请、发起 `tellaskBack`，还是另开其它诉请对话。
...
- 是否还有后台诉请 pending：只更新可观测状态；主线可被 Diligence Push 保活，支线不由运行时鞭策。
...
- `wait-group`：业务上不是 caller 正在等待一组支线，而是“同一次派发批次的回贴结果要成组收口”。因此设计用 dispatch batch / 派发批次描述，不再新增 `waitGroupId` 这类等待语义字段名。
...
- reply delivery 元信息：active reply obligation、expected reply tool、target dialog/callId、delivery status。
...
> pending tellask 只是后台进行中事实；它只参与 observability、主线 Diligence Push 是否保活、以及回贴到达后的 result-arrival handling，不决定发起方 dialog 是否继续工作。

## 当前实现事实

### 为什么大部分场景看起来符合原则

当前实现中，普通单个 `tellask` 后多数情况下会停住，但原因不是最终原则，而是旧“pending sideDialog 等同等待支线”的投影机制。
...
因此多数单个 pending `tellask` 后会因为 `canDrive=false` break。这种结果表面接近“发完诉请自然停”，但底层语义是“主线等待 sideDialogs”，与最终原则不一致。
...
- 普通路径：pending tellask 被当成“需要等待”，主线停。
- proceeding/recovery 路径：这个“需要等待”的判断被绕过，主线续推。

这解释了“多数场景符合，但该现场不符合”：多数场景靠旧误投影恰好停住；该现场被 queued/proceeding recovery 绕过了旧误投影。

更精确地说，当前实现存在一个“结果偶然正确、控制语义错误”的组合：

- 普通路径用 `pendingSideDialogs -> canDrive=false` 让主线停，这是错因对果。
- resume/proceeding 路径用 `resumeInProgressGeneration -> allowPendingSideDialogs=true` 绕过同一判断，让主线继续，这是错因错果。
...
### Gap 1：pending sideDialog 被误当成“发起方正在等待”
...
- 主线 pending tellask 被显示成 `blocked/waiting_for_sideDialogs`。
- 支线如果自己再发起下游诉请，也会被相同的 pending sideDialog 机制当成“正在等待下游”，而这同样不符合 background delegation 语义。
- Continue / resumable 计数把 pending tellask 当成暂停原因，且无法区分“发起方后台诉请 pending”和“callee active reply obligation 未完成”。
- backend loop 用 `canDrive()` 把 pending tellask 当作不可驱动条件。

目标：

- 对任意 asker dialog，pending tellask 不应导致 `canDrive=false`。
- 对任意 asker dialog，pending tellask 不应投影为 `blocked/waiting_for_sideDialogs`。
...
- `runtime.ts` 现有的 sideDialog diligence recovery 路径。
...
- 任意 dialog 作为 asker 时，它发出的 pending tellask 是后台事实，不应让自身 `canDrive=false`。
...
- 移除支线运行时鞭策语义；支线 direct-fallback 交由 caller 决策，不由 runtime 代替支线续推。
...
### Gap 3：Diligence Push 与“等待支线”投影混杂

Diligence Push 是主线编排保活机制。它可以参考 pending tellask，但不应复用“等待支线”投影，也不应扩展成支线 reply-recovery 机制。
...
2. UI 显示主线 idle 或 proceeding/保活状态，不显示 `waiting_for_sideDialogs`。
3. pending tellask 状态通过 pendingTellask reminder / pending sideDialog panel / call bubble 展示。
...
- 支线非本意直接回复停止驱动时，不注入鞭策 prompt；direct-fallback 交给 caller 处理。
...
`recoverPendingReplyTellaskCallsAfterRestart()` 当前通过扫描历史 call-without-result 推导未交付 reply。这是旧技术债；目标态应由 reply delivery 元信息记录 `deliveryStatus`，重启后直接读取状态快照恢复未完成交付。
...
回贴完成后，等待该结果的 dialog 应处理新事实，这是另一条正当自动继续路径。常见情况是支线对话回贴后让发起方对话处理结果；`tellaskBack` 中也可能是主线或其它上游 dialog 等待 ask-back result。
...
- `getSuspensionStatus()` 若保留，应改名或拆分，避免 `sideDialogs` 与 `blockingSideDialogs` 被调用方误解为同一件事。
...
- 支线 direct-fallback 不走 Diligence prompt，改为形成 caller 可判断的 result/direct-fallback fact。
...
- fork 目标 display/run-control 不再因为 copied pending sideDialogs 进入等待支线状态。
...
1. asker dialog 单个 `tellask` 后不 immediate post-tool continue。
   ...
   - 期望：主线 displayState 不为 `waiting_for_sideDialogs`。
...
14. 支线作为 callee 同时作为 asker 发起下游 `tellask`。
...
- `interruption-resumption.ts` 中依赖 `waiting_for_sideDialogs` 的断言需要按新语义拆分 asker background work / callee completion obligation。
- `sideDialog-mixed-tool-round-honors-suspension` 重命名为正向业务语义测试：pending sideDialog 不让 asker dialog 暂停，active reply obligation 仍参与完成/恢复判定。
- run-control visual / counts 需要更新 pending tellask 不计入暂停/可恢复计数。
- fork snapshot 中 pending sideDialogs 仍要复制为 background facts，但不投影为等待支线状态。
...
4. 调整 dialog `getSuspensionStatus()` 或新增 next-step facts API，先让 drive 使用新 API；API 必须按角色区分 asker background work 与 callee completion obligation。
5. 调整 display projection，移除 pending tellask -> `waiting_for_sideDialogs`，覆盖主线和支线。
...
- pending tellask 存在时，发起方 dialog 可被驱动，但不是因为 pending tellask 必须驱动。
- pending tellask 存在时，发起方 dialog 可 idle，但 idle 不代表后台完成。
