...
本提交是第一阶段语义对齐，不是完整状态机终态。WIP 标记保留到下一次完整状态机提交完成后再移除。
...
- `NextStepTriggerState` 尚未落地；`needsDrive` 仍是 boolean / registry 投影，尚不能追溯到完整 trigger union。
- durable `dispatchBatchId` 尚未落地；当前多路 tellask 仍主要依赖 `callSiteCourse` / `callSiteGenseq` / `callId` 与 no-prompt entitlement 表达批次边界。
- `generationRunState` 尚未落地；restart proceeding recovery 仍以 `latest.generating` / `isRecoverableGeneratingLatest()` 为主。
- reply delivery status 尚未落地；`reply-special` restart recovery 仍通过扫描当前 course events 查找未完成 `replyTellask*`。
