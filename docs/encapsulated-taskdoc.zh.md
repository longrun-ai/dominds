...
- **持久性**：使任务状态在长期工作和对话轮次重置之间保持持久。
- **可审计性**：使任务变更明确且可归因于单一、有意的操作。
- **安全性**：防止绕过预期用户体验和控制点的意外或工具驱动的读/写。
- **可共享性**：确保任务文档在整个对话树（根 + 子对话）中可见/一致。

## 非目标

- 替换提醒、Q4H 或代理内存（这些仍然是单独的机制）。
- 设计通用项目管理格式（工单、史诗、多任务看板）。
- 支持任务文档包内的任意二进制资产（超出 v1 范围）。

## 术语

- **对话树**：根对话加上在其下产生的任何子对话/队友。
- **任务文档包**：以 `.tsk/` 后缀结尾的目录，将任务文档存储为多个文件。
- **任务文档（有效）**：呈现给代理的逻辑任务文档内容，从任务文档包派生而来。
- **封装**：将 `.tsk/` 视为受保护的内部状态，而不是普通工作区文件。

## 任务文档包结构

任务文档包是后缀为 `.tsk/` 的目录（例如 `my-task.tsk/`），包含：
...
Dominds 可以在任务文档包内支持一个特殊的子目录 `bearinmind/`。
...
运行时可以为内部需要在任务文档包内存储额外文件（例如：`snapshots/`、`attachments-index.md`）。
...
Dominds 必须从任务文档包构造一个**有效任务文档**用于提示词和 UI 显示。

规范结构（v1）：

1. 稳定的标题，指示这是对话树的任务文档
...
- 有效任务文档必须是确定性的（除了上述框架之外没有隐藏的重新格式化）。
- 允许空部分，但文件仍然存在。

## `change_mind` 语义（不重置轮次）

函数工具 `change_mind` 通过**替换其整个内容**来更新任务文档包的**恰好一个**部分文件。

关键点：

- `change_mind` **不得**开始新的对话轮次。
- 如果需要轮次重置，单独调用函数工具 `clear_mind({ "reminder_content": "<re-entry package>" })`（或其他轮次控制机制）。
  - 建议：包含一个简短的、可扫描的重入包，以便代理在新轮次后能够恢复。

### 参数（当前）

`change_mind` 通过**替换其整个内容**来更新**恰好一个**任务文档部分。
...
当提供 `category` 时，`selector` 指向任务文档包内 `<category>/` 下的文件。
...
- 成功的 `change_mind` 会立即更新任务文档包，并对以下内容可见：
  - 当前对话
  - 对话树中的所有子对话/队友
  - 任何观察的 WebUI 客户端
...
- 防止通过通用文件操作进行意外编辑。
- 强制通过明确的、有语义的操作（`change_mind`）进行任务文档变更。
- 避免代理"好心"地在没有明确意图的情况下重写任务约束这类提示词/控制流陷阱。

系统提示词（和向代理展示的任何工具文档）必须明确说明此限制。

## UX 预期（设计级）

- WebUI 应将任务文档渲染为三个窗格/选项卡（目标/约束/进度）和可选的组合视图。
- WebUI 应使编辑明确：当用户更改一个部分时，必须清楚正在替换哪个部分。
- WebUI 应显示每个部分的简短"最后更新"指示器（时间 + 参与者）以支持可审计性。

## 兼容性考虑

Dominds 应将 `*.tsk/` 任务文档包标准化为**唯一**支持的任务文档存储格式。

如果工作区以前使用单文件 `.md` 任务文档，它们必须在运行新对话之前迁移到 `*.tsk/` 包。

## 安全与完整性说明

- `.tsk/` 包是**高完整性状态**：它可以实质性地改变代理行为和安全边界。
- 封装减少了隐蔽编辑的机会（例如，通过复制文本中嵌入的工具调用）。
- 强烈建议使用审计元数据（谁/何时）进行事件分析和用户信任。

## 开放问题

- 任务文档包默认应该放在哪里（在对话持久化下、在启动入口旁边，还是专用工作区目录）？
