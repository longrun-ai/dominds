...
`@human` (`!?@human ...`). When a dialog enters Q4H, progression is suspended until the human responds.
...
1. **External deep links** that jump directly to a Q4H question’s call site (the place in the conversation where
   `!?@human` was issued), and optionally **pre-select** the pending question so the input enters “answer mode”.
...
- **Call site**: the location in the conversation where a Tellask was issued (for Q4H: where `!?@human` appears).
...
- `callId`: tellask callId when the Q4H was created from an `!?@human` tellask block
...
- Add optional `callId?: string` to Q4H questions created from an `!?@human` tellask block.
