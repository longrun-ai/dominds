# Encapsulated Task Documents (`*.tsk/`) (Design)

This document specifies a **structured, encapsulated task document format** for Dominds dialogs.

Instead of a single mutable “task doc” markdown file, each dialog tree’s task doc becomes a **task package directory**
...
- **Shareability**: ensure the task doc is visible/consistent across the entire dialog tree (root + subdialogs).
...
- Supporting arbitrary binary assets inside task docs (out of scope for v1).

## Terminology

- **Dialog tree**: a root dialog plus any subdialogs/teammates spawned under it.
- **Task package**: a directory ending in `.tsk/` that stores the task doc as multiple files.
- **Task doc (effective)**: the logical task doc content presented to the agent, derived from the task package.
- **Encapsulation**: treating `.tsk/` as protected internal state, not general workspace files.

## Task Package Structure

A task package is a directory with the suffix `.tsk/` (e.g. `my-task.tsk/`), containing:
...
## Effective Task Doc (for agent context)

Dominds MUST construct an **effective task doc** from the task package for use in prompts and UI display.

Normative structure (v1):

1. A stable heading indicating this is the task doc for the dialog tree
...
- The WebUI SHOULD render the task doc as three panes/tabs (Goals / Constraints / Progress) and optionally a combined view.
...
Dominds SHOULD standardize on `*.tsk/` task packages as the **only** supported task doc storage format.

If a workspace previously used single-file `.md` task docs, they MUST be migrated to `*.tsk/` packages before running new dialogs.
