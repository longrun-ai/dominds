...
- 自动完成/自动将对话标记为完成。
- 将鞭策语注入应用于支线对话。支线对话应由普通业务事实继续推进，或向诉请者发送诉请回复；非本意的直接回复停止由 direct-reply fallback 与 caller 后续判断处理。
