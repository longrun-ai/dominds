...
- **自动续推注入**：仅针对**主线对话**，当驱动程序即将停止时，运行时会自动发送一个简短的鞭策语（渲染为正常的用户气泡）并继续生成，除非对话处于 Q4H 等真正等待用户输入的状态；pending tellask 只作为鞭策判断输入与文案上下文。
...
鞭策恢复注入仍仅作用于主线对话。active callee dispatch 不应否决这一次鞭策注入；普通鞭策同样把 active callee dispatch 当作上下文，而不是暂停理由。支线对话 retry-stopped 恢复不注入鞭策语；后续由普通业务事实、direct-reply fallback 或 caller 的下一步判断决定。
...
1. 如果对话正在等待 Q4H，则停止；active callee dispatch 是后台被诉请者事实，可作为鞭策上下文，不是暂停理由。
2. 如果有任何工具反馈，则正常继续。
