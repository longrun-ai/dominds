...
- 提供可复核输出：YAML summary + evidence（plan）/apply_evidence（apply） + unified diff；pad-sourced hunk 为避免回显大块正文，使用 redacted 输出例外。
...
- 当前已实现：`pad_write`、`pad_load_file_range`、`pad_edit`、`pad_insert`、`pad_delete_range`、`pad_copy`、`pad_move`、`pad_prepare_file_range_edit`、`pad_delete`。
...
- `pad_prepare_file_range_edit` 使用 pad 内容创建 hunk，但结果不回显 pad 正文，也不输出包含新内容的 diff；确认无误后仍通过 `apply_file_modification` 落盘。由 pad 创建的 hunk 在 apply 成功时也使用 redacted 输出，不回显新内容片段或 diff。
