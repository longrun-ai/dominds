...
- 当前已实现：`pad_write`、`pad_load_file_range`、`pad_edit`、`pad_insert`、`pad_delete_range`、`pad_copy`、`pad_move`、`pad_prepare_file_range_edit`、`pad_delete`；文件侧已实现 `file_range_edit` 直接消费 pad。
...
- 文件编辑工具应能直接把 pad 当作源或目标：文件范围可装入 pad，pad 范围可作为 `file_range_edit` 的新内容，pad 之间可复制/剪切/移动文本。
...
- 第二档：继续扩展文件工具对 pad 的直接源/目标支持。已完成：`create_new_file` / `overwrite_entire_file` / `file_range_edit` 支持 `pad_id/pad_range` 作为来源，同时保留 `content` 直供。待做：文件全量/范围直接写入 pad，以及将文件编辑工具的输出目标改为 pad 的变体。落地时仍保持“无 pad_read / 无观察工具”原则。
