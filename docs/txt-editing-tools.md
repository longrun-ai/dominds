...
- **batch occurrence replace**：只有替换同一字面量的两个及以上 occurrence 时，保留 `prepare_occurrence_replace` → `apply_occurrence_replace` 两步流程。
...
- 把 prepare/apply 严格限制到多 occurrence 批量字面量替换：`prepare_occurrence_replace` / `apply_occurrence_replace`。
...
- `prepare_occurrence_replace` / `apply_occurrence_replace`：单文件内的多 occurrence 字面量批量替换。默认替换所有 occurrence，也可通过 `occurrence_indexes` 指定 1-based occurrence 列表；少于两个选中 occurrence 会拒绝，避免成为单点编辑旁路。apply 时若文件 hash 已变化，则拒绝并要求重新 prepare。
...
- `NOT_MULTI_OCCURRENCE`：选中的 occurrence 少于两个；单点编辑应使用 direct file 工具。
...
- Scratch Pad is a staged `ws_mod` buffer system for large text editing. The current foundation includes `pad_write`, `pad_load_file_range`, `pad_edit`, `pad_insert`, `pad_delete_range`, `pad_copy`, `pad_move`, and `pad_delete`; `pad_load_file_range({pad_id,path})` loads whole files by default, and `create_new_file`, `overwrite_entire_file`, `file_range_edit`, `file_append`, `file_insert_*`, and `file_block_replace` can consume pad content directly. It keeps `add_reminder` / `update_reminder` / `delete_reminder` semantics unchanged, exposes pad state as prominent managed reminder metadata, and intentionally avoids pad observation tools such as `pad_read` / `pad_preview` / `pad_diff`. Large generated content may still enter persistent tool-call history when first written, but subsequent file/pad operations should move text by handle instead of re-emitting it.
