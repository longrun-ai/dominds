...
- `overwrite_entire_file`（函数工具）：整文件覆盖写入（**不走 prepare/apply**）。  
  ...
  设计定位：用于“新内容很小（例如 <100 行）”或“明确为重置/生成物”的场景；大块确定范围改动优先 pad + `file_range_edit`。
...
### 8.6 read_file / overwrite_entire_file（结构化头部）
...
- 第二档：继续扩展文件工具对 pad 的直接源/目标支持。候选包括 `overwrite_entire_file` 从 pad 覆盖、`create_new_file` 从 pad 创建、文件全量/范围直接写入 pad，以及将文件编辑工具的输出目标改为 pad 的变体。落地时仍保持“无 pad_read / 无观察工具”原则。
