...
## 1. 背景：为什么从 prepare-first 调整为 direct range edit + prepare/apply
...
- **prepare/apply for uncertain targets**：锚点、多 occurrence、候选不唯一、必须先审阅 hunk 的编辑继续走 `prepare_*` → `apply_file_modification`。
- **single apply for planned edits**：所有计划类编辑仍仅通过 `apply_file_modification({ "hunk_id": "<hunk_id>" })` 落盘。
...
- 提供可复核输出：直接 range edit 默认 YAML-only；显式 `preview/show_diff` 才输出 diff。plan/apply 继续输出 YAML summary + evidence（plan）/apply_evidence（apply） + unified diff；pad-sourced 写入默认 redacted，避免回显大块正文。
- 明确并发/时序约束：避免在同一条消息中把 prepare 与 apply 混在一起；同一文件的写入工具在进程内串行化。
...
### 4.3 增量编辑（direct range edit + prepare/apply）
...
- `create_new_file` / `overwrite_entire_file` / `file_range_edit` 都支持 `content` 与 `pad_id/pad_range` 两类来源；小正文直供 `content`，大正文优先使用 pad 来源以减少重复历史注入。
...
- 若目标是精确行号范围且不需要 hunk 预览，直接用 `file_range_edit`，不要人为拆成 prepare/apply。

### 5.2 写入并发安全（当前实现）

- 同一文件的多个 `apply_file_modification` 会在进程内按队列串行化（按 `createdAtMs`、再以 `hunkId` 作为 tie-breaker）。
- 同一文件的多个 `file_range_edit` 也会进入同一类队列串行化（按入队时间、再以内部 tie-breaker）。
- 不同文件的写入可并行，不共享锁。
...
- `apply_evidence`（普通 hunk 必须；pad-sourced hunk 可为 redacted）
...
- 当前已实现：`pad_write`、`pad_load_file_range`、`pad_edit`、`pad_insert`、`pad_delete_range`、`pad_copy`、`pad_move`、`pad_prepare_file_range_edit`、`pad_delete`；文件侧已实现 `create_new_file` / `overwrite_entire_file` / `file_range_edit` 直接消费 pad。
...
- 文件编辑工具应能直接把 pad 当作源或目标：文件范围可装入 pad，`pad_load_file_range({ pad_id, path })` 省略 `range` 时默认装入全文件；pad 范围可作为 `create_new_file` / `overwrite_entire_file` / `file_range_edit` 的新内容；pad 之间可复制/剪切/移动文本。
- `pad_prepare_file_range_edit` 使用 pad 内容创建 hunk，但结果不回显 pad 正文，也不输出包含新内容的 diff；仅在必须先保留 hunk 预览时使用。由 pad 创建的 hunk 在 apply 成功时也使用 redacted 输出，不回显新内容片段或 diff。
...
- 第二档：继续扩展文件工具对 pad 的直接源/目标支持。已完成：`pad_load_file_range` 支持文件全量/范围写入 pad；`create_new_file` / `overwrite_entire_file` / `file_range_edit` 支持 `pad_id/pad_range` 作为来源，同时保留 `content` 直供。待做：将更多文件编辑工具的输出目标改为 pad 的变体。落地时仍保持“无 pad_read / 无观察工具”原则。
...
本设计文档的 direct range edit + prepare/apply 心智模型保持一致，但路径与权限语义由 team_mgmt 工具包装层决定（详见 team_mgmt 文档/工具说明）。

---

# English (summary; derived from the Chinese doc above)
...
- Ambiguous, anchor-based, multi-occurrence, or preview-first edits remain **prepare/apply** (`prepare_*` returns YAML + unified diff + `hunk_id`; `apply_file_modification({hunk_id})` is the only apply entrypoint for planned edits).
...
- Same-file writes are serialized in-process (`apply_file_modification` and `file_range_edit` both use per-file queues).
...
- Scratch Pad is a staged `ws_mod` buffer system for large text editing. The current foundation includes `pad_write`, `pad_load_file_range`, `pad_edit`, `pad_insert`, `pad_delete_range`, `pad_copy`, `pad_move`, `pad_prepare_file_range_edit`, and `pad_delete`; `pad_load_file_range({pad_id,path})` loads whole files by default, and `create_new_file`, `overwrite_entire_file`, and `file_range_edit` can consume pad content directly. It keeps `add_reminder` / `update_reminder` / `delete_reminder` semantics unchanged, exposes pad state as prominent managed reminder metadata, and intentionally avoids pad observation tools such as `pad_read` / `pad_preview` / `pad_diff`. Large generated content may still enter persistent tool-call history when first written, but subsequent file/pad operations should move text by handle instead of re-emitting it.
