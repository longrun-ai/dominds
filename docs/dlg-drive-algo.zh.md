...
- root-local `wake-queue.json` 存储业务命名 wake queue entries，避免 backend loop 全量扫描历史支线。
...
当前 `wake-queue.json` 是 root-local durable wake queue 存储：它解决“backend loop 不能全量扫描 root 下所有历史支线”的性能与安全问题，并以 `targetDialogId`、`kind`、业务 identity 定位 handler 附近。它仍然只负责可达性，不能成为业务事实或消费账本。
