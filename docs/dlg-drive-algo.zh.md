...
`drive-watch` 的问题类似。它把某个 dialog 标成“值得被 drive 看一下”，但这仍然是 dialog-level 的技术抽象。它既不是业务事实，也不是消费账本。残留 watch 很容易变成空 drive、误 revive 或噪声日志。
...
- root-local `drive-watch` 作为迁移期 sideline wake index 已存在，避免 backend loop 全量扫描历史支线。
...
- 支线 caller 收到 callee 回贴后没有 revive 的结论保留为 wake queue 规则：不能全量扫描历史支线，必须有 root-local 精确 wake 机制；但迁移目标不是强化 `drive-watch`，而是把它降级/替换为业务命名 wake cue。
...
当前 `drive-watch` 是迁移期 root-local sideline wake index：它解决“backend loop 不能全量扫描 root 下所有历史支线”的性能与安全问题，但不应继续升级成业务概念。后续重构应把它拆成业务命名 wake cue；在迁移完成前，新代码不得增加 `drive-watch` 的业务判定职责。
...
旧的 drive-watch 可以作为迁移期 queue 存储细节存在，但新代码不应扩大它的语义。
...
6. 删除或降级 `drive-watch` 命名，保留必要的 queue 存储实现。
...
- root-local sideline wake：不能全量扫描历史支线；当前 drive-watch 只是迁移期实现，目标是业务命名 wake cue。
