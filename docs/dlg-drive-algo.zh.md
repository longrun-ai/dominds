...
`wake cue storage` 的问题类似。它把某个 dialog 标成“值得被 drive 看一下”，但这仍然是 dialog-level 的技术抽象。它既不是业务事实，也不是消费账本。残留 watch 很容易变成空 drive、误 revive 或噪声日志。
...
- `docs/issues/tellask-background-continuation-live-bugs-2026-05-17.zh.md`

前两份文档记录的是 tellask background continuation 从现场 bug 到第一轮状态机收敛的过程。当前代码已经完成其中大部分基础：
...
- root-local `wake cue storage` 作为迁移期 sideline wake cue 存储细节已存在，避免 backend loop 全量扫描历史支线。
...
当前文档应保留前序文档中仍有长期价值的原则；前序文档中的现场路径、阶段性 patch 计划、已完成 bug 记录，可以在本文补齐后交给 git 历史归档。
...
`tellask-background-continuation-live-bugs-2026-05-17.zh.md` 中的现场 bug 信息应按以下方式沉淀：
...
- 支线 caller 收到 callee 回贴后没有 revive 的结论保留为 wake queue 规则：不能全量扫描历史支线，必须有 root-local 精确 wake 机制；但迁移目标不是强化 `wake cue storage`，而是把它降级/替换为业务命名 wake cue。
- `needsDrive` / `backend_queue` 旧术语继续收敛为显式 wake cue 与 next-step trigger，不再作为文档概念保留。
...
队列项应该逐步从 dialog-level watch 迁移为业务命名的 wake cue，例如：
...
这些 cue 仍然只是唤醒提示，不是业务权威。handler 必须重新读取 durable state 并本地 claim。

当前 `wake cue storage` 是迁移期 root-local sideline wake cue 存储细节：它解决“backend loop 不能全量扫描 root 下所有历史支线”的性能与安全问题，但不应继续升级成业务概念。后续重构应把它拆成业务命名 wake cue；在迁移完成前，新代码不得增加 `wake cue storage` 的业务判定职责。
...
如果一个 dialog 被唤醒但没有任何 handler 能 claim 业务事实，这不是“尝试 no-prompt drive”的理由。应移除或过期对应 wake cue，并保留足够 diagnostics。
...
`userWait.kind === awaiting_user_answer` 这类先决等待不是 trigger，也不是业务 continuation 的消费点。等待用户期间到达的 `result_arrival`、`reply_delivery_recovery`、`followup` 等事实仍可写入 durable state / wake cue，但 backend driver 不能越过 user wait 自动启动 generation。
...
业务上不存在一个叫“这个 dialog 需要 drive”的 boolean。真实来源必须是具体 wake cue / next-step trigger / open generation / pending reply delivery 等结构化事实。
...
跨 root 找到 sideline 的唤醒机制仍然需要，但不应叫 watch dialog，也不应表达“这个 dialog 应该被 drive”。目标是迁移为业务命名 wake cue。

旧的 wake cue storage 可以作为迁移期 queue 存储细节存在，但新代码不应扩大它的语义。
...
4. 把 root/sideline wake 存储从 dialog-level watch 迁移到业务命名 wake cue。
5. 删除 `noPromptSideDialogResumeEntitlement` 和 `inspectNoPromptSideDialogDrive`。
6. 删除或降级 `wake cue storage` 命名，保留必要的 queue 存储实现。
7. 收敛 malformed 边界：必要运行元信息缺失时 loud diagnostic / malformed，而不是初始化默认值后继续运行。
8. 删除前序两份阶段性文档，让本文成为后续 dialog drive 重构的唯一设计入口。
...
删除 `tellask-background-continuation-refactor.zh.md` 和 `issues/tellask-background-continuation-live-bugs-2026-05-17.zh.md` 前，应确认以下信息已在本文、代码、测试或 git 历史中有归宿：
...
- root-local sideline wake：不能全量扫描历史支线；当前 wake cue storage 只是迁移期实现，目标是业务命名 wake cue。
- 状态机元信息：常态 runtime 不回扫 course JSONL；缺元信息时 loud/malformed。
- 已完成现场 bug 的测试名或代码路径保留，例如 background callee badge event、sideDialog caller result-arrival backend watch、reply delivery duplicate/stale handling。
