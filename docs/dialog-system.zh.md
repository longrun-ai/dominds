...
- 诉请回复会路由到 `assignmentFromAsker` 中记录的**当前诉请者**。
...
**subdlg.yaml** 文件索引诉请者名下仍未发送诉请回复的后台被诉请者对话。与 `q4h.yaml` 类似，它是一个索引文件，不是真理之源：

- 索引跟踪哪些支线对话 ID 仍负有向 caller 发送诉请回复的义务
- 实际的支线对话状态从磁盘 (done/ 目录) 验证
- 用于后端协程的崩溃恢复、可观测状态和结果到达处理
...
2. 对于每个候选者，检查具体驱动来源（Q4H 已回答、queued prompt、结果到达、open generation 恢复等）
3. 在驱动之前获取对话的互斥锁
4. 执行生成循环直到自然 idle、进入 Q4H 或完成
5. 释放互斥锁
6. 将所有状态变更持久化到存储

驱动循环持续进行，直到对话自然 idle、进入 Q4H 或完成。active callee dispatch 是后台被诉请者事实，不会单独挂起 caller；但只有 pending active callee dispatch、没有其它具体驱动来源时，caller 应自然 idle，不靠鞭策语维持主线空转。当用户回答 Q4H 或 callee 结果到达时，后端记录该具体事实，并仅从该事实驱动受影响对话。
...
4. 模型也可以产生结构化 `answering` 输出（或调用 `answerHuman({ answerContent })`）来表达“这是给人类看的答复或状态说明”。该输出无条件追加到该对话的 `a2h.yaml`，但它不等价于支线对话对诉请者的正式诉请回复。
5. 如果插话答完后仍有跨对话回复义务，它保持为普通的 durable reply-obligation 状态；后续由正常业务路径推进：queued prompt、诉请回复提醒、鞭策续推，或在对话确实阻塞时由显式 resume 触发。
...
**关键点**：“是否还有用户插话待可见回复”和“是否还有跨对话回复义务”是两个独立业务事实。提醒项 footer 可以直接用这两个事实定制：插话尚未答复时优先答人；插话已答复且回复义务仍 active 时，继续按跨对话诉请回复要求收口。
...
- 支线对话没有独立的活性维持状态。支线继续推进，是因为正常业务状态仍未完成：工具结果需要处理、Q4H 待答、仍在等待下游 callee 的诉请回复，或已有显式 runtime/user prompt 排队。不要新增只为维持活跃而存在的旁路。
...
- 运行时不得通过回扫历史消息文本来推断这个尾部状态。历史上下文属于模型和 UI；调度状态必须由当前 drive result、active reply obligation、pending Q4H 索引或 queued prompt 等显式状态携带。
...
- 若诉请方在正文中定义“诉请回复格式/交付格式”，只写业务交付结构即可；不得要求被诉请者手写任何标记，因为这些标记由运行时自动注入。
...
结果：
- sub-001 记录回问诉请并等待诉请者答复
- 驱动程序使用问题驱动 orchestrator
- orchestrator 用指导响应
- sub-001 带着 orchestrator 的响应继续
```
...
4. 诉请者记录后台被诉请者事实；不会仅因 active callee dispatch 被阻塞
5. 支线对话的响应流回诉请者，形成结果到达事实
6. 诉请者可因结果到达、queued prompt、用户输入、无 pending active callee 时的 Diligence Push 或其它显式驱动源继续
...
- 在主线的注册表中注册它
- orchestrator 记录后台被诉请者事实
- 驱动 researcher 支线对话
- 响应作为结果到达事实流回 orchestrator
...
1. 当前对话记录后台被诉请者事实；不会仅因 active callee dispatch 被阻塞
...
4. 支线对话的响应作为结果到达事实流回诉请者
5. 诉请者可因结果到达、queued prompt、用户输入、无 pending active callee 时的 Diligence Push 或其它显式驱动源继续
...
- code-reviewer 完成，审查结果作为结果到达事实流回 orchestrator
...
| **注册**         | 不适用                                   | 创建并注册                                                                               | 永不注册                                                                                  |
| **诉请者行为**   | callee 等待诉请者回问回复                | 诉请者记录后台被诉请者事实                                                               | 诉请者记录后台被诉请者事实                                                                |
...
5. **callee 响应供应**：callee 通过持久化将响应写入*当前诉请者*的上下文（不是回调）。对于 TYPE B，每次诉请都会用最新的诉请者 + tellaskInfo 更新支线对话的 `assignmentFromAsker`，因此响应被路由到最近的诉请者（主线或支线对话）。这支持分离操作、复用和崩溃恢复。

   TYPE B 长线诉请更新的确认边界必须谨慎区分：pending 状态与 `asker-stack.jsonl` 更新，只表示运行时已接受/登记新要求；只有 callee driver 实际消费 `registered_assignment_update` up-next、写入可见更新气泡和 assignment anchor 后，才表示目标 callee 已收到并开始处理更新。`registered_assignment_update` up-next 是运行态调度状态，不是持久化 redo log；不要为了“更新气泡”即时可重放而新增严格持久队列。后端进程重启后的业务自愈依赖 loud facts 与足够上下文（pending reminder、asker stack 最新 assignment、旧 call 是否缺少终态、目标 callee 是否缺少 assignment anchor），让 LLM 自主决定重发、修正或继续观察后台事实，而不是追求技术层面绝不丢内存队列。

6. **支线对话注册表**：已注册的支线对话（TYPE B 长线诉请）在主线对话作用域的注册表中跟踪。注册表在 `clear_mind` 操作中持续存在，并在主线加载时重建。

7. **状态保留契约**：
   - `clear_mind`：清除消息，清除 Q4H 索引，保留提醒，保留注册表
   - callee 完成：向当前诉请者写入响应，从待处理列表中删除（注册表不变）
...
sequenceDiagram
  ...
  Driver-->>Side: 支线对话恢复
  Side-->>Asker: 供应结果到达事实（清除该 pending callee）
```
...
当 callee 提出 Q4H 并等待人工输入时，caller 的 result-arrival 处理必须尊重“还没有 callee 结果”这个事实：

```typescript
// caller 检查 callee 完成状态
async function checkCalleeResultArrival(caller: Dialog): Promise<void> {
  const pending = await loadActiveCalleeDispatches(caller.id);

  for (const p of pending) {
    // 检查 callee 是否有未解决的 Q4H
    const sideDialogQ4H = await DialogPersistence.loadQuestions4HumanState(p.sideDialogId);

    if (sideDialogQ4H.length > 0) {
      // callee 正在等待人工输入；此时还不存在结果到达事实
      log.debug(`callee ${p.sideDialogId} 有 ${sideDialogQ4H.length} 个 Q4H，跳过结果到达处理`);
      continue;
    }

    // callee 没有 Q4H，检查是否完成
    const isDone = await isSideDialogCompleted(p.sideDialogId);
    if (isDone) {
      // 合并响应，并从这个具体事实触发后续调度
      await incorporateSideDialogResponse(caller, p.sideDialogId);
    }
  }
}
```
...
- 如果新的 TYPE B 诉请在上一轮回复前到达，运行时会立刻用一条系统生成的失败诉请结果结束上一轮等待；文案必须从业务事实描述，不使用协议/实现术语。
...
- 如果支线对话在最新 assignment 提示真正落到本地之前先产出了回复，这条回复不得作为新一轮诉请结果送达诉请者。
...
- `restore_pending`：若切点之前存在待处理 Q4H，则恢复该用户等待状态；active callee dispatches 复制为后台被诉请者事实，不投影为阻塞态
- `auto_continue`：若切点之前没有待处理 Q4H 等用户等待事实，则新主线以 `interrupted(system_stop: fork_dialog_continue)` 初始化，UI 随后立即发送 `resume_dialog`
...
sequenceDiagram
  ...
  Asker-->>Driver: 响应文本
  Driver-->>Side: 驱动 callee，诉请者响应在上下文中
```
...
`callId` 的呼应点。正常 callee 回复、跨程 carryover、已注册会话被更新后的替代通知都可以结束这次
call；不能静默删除或覆盖 pending 轮次。已注册会话被更新时，替代通知的文案不能在目标 callee 实际消费
更新前宣称“更新已送达/目标 callee 正在按新要求处理”；在此之前只能表达“旧轮已被新要求登记替换，目标
callee 尚未在安全边界消费更新”。
...
sequenceDiagram
  ...
  Driver->>Side: 驱动
  Side-->>Driver: 最终响应
  Driver-->>Tellasker: 供应结果到达事实（无注册表更新）
```
...
sequenceDiagram
  ...
  Asker->>Side: 创建支线对话（TYPE B 或 C）
  Note over Asker,Side: 诉请者记录后台被诉请者事实
...
sequenceDiagram
  ...
  Asker-->>Asker: 记录后台被诉请者事实；只有 Q4H 是用户等待阻塞点
