...
驱动循环持续进行，直到对话自然 idle、进入 Q4H 或完成。pending sideDialogs 是后台被诉请者事实，不会单独挂起 caller。当用户回答 Q4H 或 callee 结果到达时，后端记录该具体事实，并仅从该事实驱动受影响对话。
...
4. 诉请者记录后台被诉请者事实；不会仅因 pending sideDialog 被阻塞
5. 支线对话的响应流回诉请者，形成结果到达事实
6. 诉请者可因结果到达、queued prompt、用户输入、Diligence Push 或其它显式驱动源继续
...
1. 当前对话记录后台被诉请者事实；不会仅因 pending sideDialog 被阻塞
...
5. 诉请者可因结果到达、queued prompt、用户输入、Diligence Push 或其它显式驱动源继续
...
async function checkCalleeResultArrival(caller: Dialog): Promise<void> {
  const pending = await loadPendingSideDialogs(caller.id);
...
- `restore_pending`：若切点之前存在待处理 Q4H，则恢复该用户等待状态；pending sideDialogs 复制为后台被诉请者事实，不投影为阻塞态
...
- 派生的门控：`hasPendingQ4H()`；pending sideDialogs 是后台被诉请者事实，不是驱动门控
