...
**诉请者**是发起本次诉请的对话（caller），**被诉请者**是处理本次诉请的对话（当前对话）。这是**调用角色**，不是层级关系：
...
```mermaid
sequenceDiagram
  participant Requester as 诉请者
  participant Driver as 后端驱动程序
  participant Reg as 主线支线对话注册表
  participant Side as 已注册的支线对话

  Requester->>Driver: 发出 `tellask({ targetAgentId: "agentId", sessionSlug: "tellaskSession", tellaskContent: "..." })`
  Driver->>Reg: 查找 `agentId!sessionSlug`
  alt 注册表命中
    Reg-->>Driver: 现有支线对话 selfId
    opt 前一轮仍在等待
      Driver-->>Requester: 用系统生成的业务化文案结束前一轮等待
  ...
  Side-->>Driver: 最终响应
  Driver-->>Requester: 供应响应 + 清除待处理支线对话
  opt 诉请者是主线且现在已解除阻塞
    Driver-->>Requester: 设置 needsDrive=true（自动重启调度）
  end
```
...
sequenceDiagram
  ...
  Requester->>Driver: 发出 tellaskSessionless(...)
  Driver->>Side: 创建（不注册）
  Driver->>Side: 驱动
  Side-->>Driver: 最终响应
  Driver-->>Requester: 供应响应（无注册表更新）
```
