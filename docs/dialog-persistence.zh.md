...
- 差遣牒是标准 rtws 工件，不是对话特定存储
- 多个对话可以引用相同的差遣牒进行协作推进
- 差遣牒在整个 DevOps 生命周期中持续存在，超出单个对话
- 差遣牒文件的更改对所有引用它的对话立即可见
- 把差遣牒当作共享公告板：关键决策/当前状态/下一步写回 `progress`；硬约束写回 `constraints`（不要只留在对话/提醒项里）
