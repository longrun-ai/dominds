...
英文版：[English](./mottos.md)

**TL;DR** 需要实现社会分工，涉及到 AI 智能体
...
- **中央差遣牒（Taskdoc）**：任务实时协调的公告板；目标、约束和进度的单一真实来源
- **动态上下文窗口**：时间限制的上下文保留
...
- 使用 `@self`（不带 `!tellaskSession`）进行默认的"初心"会话：隔离一个子问题，回答，然后返回。
- **很少**使用 `@self !tellaskSession <tellaskSession>`，仅当你明确想要一个**可恢复的**长期初心会话用于多步骤子问题时。
...
关键洞察是，AI 智能体和人类一样，从心理卫生实践中受益匪浅。定期的认知清理和专注的问题解决会话可以显著提高智能体推理的质量和效率。

这种方法将管理大量对话上下文的挑战转化为优势，允许智能体在保持中央差遣牒整体项目连续性的同时，对复杂问题的每个方面带来新鲜、专注的注意力。
