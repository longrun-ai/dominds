...
当前 prototype 说明（`dominds-apps/@longrun-ai/web-dev`，截至 2026-03-08）：

- 该 app 已可安装，并已贡献 `web_tester` / `web_developer` teammate 与可用的 `playwright_interactive` toolset 注册。
- installable app id 当前固定为 `@longrun-ai/web-dev`；本地开发目录与 npm package 也保持同一 scoped identity，避免多套命名并存。
