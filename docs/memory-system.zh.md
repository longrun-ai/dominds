...
- **在队友之间共享**（在同一工作区/任务文档内）：每个队友/子对话看到相同的任务文档（`goals` / `constraints` / `progress`）。
...
**目的**：特定智能体人格的持久"我如何工作"知识，加上一个紧凑的责任领域"工作区索引"，这样你就可以在不重新读取文件的情况下采取行动。
...
- 启发式，
- 稳定偏好。
- **责任领域工作区索引**：你拥有的精确文件路径（文档和/或代码），加上最小的关键事实（入口点、关键符号、局部契约），让你可以直接提出和应用编辑而不必重新读取工作区。
...
- 将你的责任领域工作区索引视为你的范围内的精炼"单一真实来源"。
- 每当你更改相关文件或检测到过时/冲突时，立即更新个人记忆（`replace_memory`），以便它跟踪最新的工作区事实。
