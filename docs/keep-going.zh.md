# Keep-Going（勤奋自动继续）— 设计文档

## 概述

Domains 根对话旨在长期运行。根对话"停止"（变为空闲）通常不是操作员想要的：他们希望智能体持续推进，直到：

- 合法地暂停等待人类决策（Q4H），或
- 合法地暂停等待子对话（tellask/backfill）。

本文档指定了一个运行时机制（"keep-going"），仅针对**根/主对话**，防止对话停止：每当驱动程序即将停止时，它会自动发送一个简短的勤奋提示（渲染为正常的用户气泡）并继续生成，除非对话处于合法暂停状态（Q4H 或待处理的子对话）。

## 目标

- 防止根对话停止，除非处于合法暂停状态（Q4H / 子对话）。
- 保持行为可预测和有界（无无限循环）。
- 使提示文本可按工作区（rtws）和语言配置。
- 提供清晰的用户控制的"禁用"机制。
...
运行时自动发送一个勤奋提示（渲染为正常的用户气泡）并运行另一次生成迭代。

### 有界性

为避免无限循环，keep-going 有一个按对话的预算（每个成员的 `diligence-push-max`），控制对于给定对话在运行时强制 Q4H 暂停之前可以注入多少个自动继续的勤奋提示。
...
- 如果选中的勤奋文件存在但其内容为空/仅空白，则禁用 keep-going（不注入）。
...
注意：勤奋文件中的 YAML frontmatter 会被运行时忽略。如果存在，它被视为非内容元数据并从提示正文中剥离。
...
- 如果 `diligence-push-max < 1`，则该成员的 keep-going 被禁用（不注入），即使勤奋文件存在。
- 内置影子成员 `fuxi` 和 `pangu` 默认为 `diligence-push-max: 0`，除非在 team.yaml 中显式覆盖。

## UX 备注

- Keep-going 是一个仅运行时的提示，但它应该是**可见的**：勤奋提示渲染为正常的用户消息气泡（由运行时自动发送），以便操作员理解为什么会出现额外的迭代。
