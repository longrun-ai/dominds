...
- 每次生成轮次计算并持久化的**上下文健康快照**（从使用情况 + 每个模型的元数据派生）。
...
### 重入包（Re-entry Package）

恢复工作流围绕一个**重入包**（一个可扫描的、可操作的上下文束）展开，该束在新轮次中存活。
...
- 每轮，驱动程序记录一条 **role=user prompt**（持久化为用户消息），在 UI 中作为用户 prompt 可见。此提示告诉代理：
  - 通过 `update_reminder` / `add_reminder` 整理提醒项（尽力而为的重入包），然后调用 `clear_mind` 开始新轮次。
- 提示包含倒计时：经过 **N** 轮后系统将自动清空。
- 当倒计时归零时，驱动程序**自动调用** `clear_mind`（带空参数；不要求 `reminder_content`），开始新轮次且无需暂停。

理由：

- `caution` 已经在提醒项中尽力推动重入包草稿的编写。
...
- v3 恢复：
  - `caution`：驱动程序插入持久化的 role=user prompt（UI 可见的用户指令）。进入 `caution` 时插入一次；保持在 `caution` 状态时按节奏重新插入（默认：每 10 次生成；可按模型配置）。每次代理必须至少调用 `update_reminder` / `add_reminder` 之一并维护重入包草稿，然后在就绪时执行 `clear_mind`。
  - `critical`：驱动程序使用**记录的角色为 user 的 prompt** 运行倒计时恢复（最多 5 轮）。每次提示包含倒计时并指示提醒整理 + `clear_mind`。当倒计时归零时，驱动程序自动执行 `clear_mind` 并开始新轮次（无 Q4H，无暂停）。
