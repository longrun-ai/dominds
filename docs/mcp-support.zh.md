...
许多真实的 MCP 服务器**并不安全，无法在多个对话/代理之间共享**。示例包括保持可变会话状态的服务器、维护隐式"当前页面"句柄的服务器，或具有全局进程范围缓存的服务器。
...
- 在首次租赁时，Dominds 向对话添加一个**粘性拥有提醒**，指示代理在确信不再需要该工具集时释放租赁。

释放：

- 代理应调用 `mcp_release({"serverId":"<serverId>"})`（来自 `mcp_admin`）来释放当前对话的租赁客户端实例。
...
由于工具注册表是全局的，MCP 工具集必须在使用 `Team.Member.listTools()` 构建代理工具列表之前注册（因此也在 `dominds/main/llm/driver.ts` 中的 LLM 生成之前）。
...
关键要求：不通过白名单/黑名单规则的工具**永远不会注册**到 Dominds 的工具系统中，因此永远不能呈现给代理使用。
...
本节设计了一个安全、实用的热重载机制，适合 Dominds 的**全局工具（集）注册表**以及代理在每轮生成中重新解析工具的事实。
...
如果成员引用了不再存在的工具集（例如 `playwright` 被移除），那么 `Team.Member.listTools()` 将警告"工具集未找到"，代理在该轮中 simply 不会有那些工具。这是可以接受的；单独的 UX 改进可以在 UI 中呈现缺失的工具集。

### 可选：版本控制

维护一个单调递增的 `mcpRegistryVersion`，在每次成功重新加载后更新。这可用于：

- 调试日志（"代理轮次使用了 MCP 版本 N"）
- UI 状态显示（"MCP 配置加载于……"）
