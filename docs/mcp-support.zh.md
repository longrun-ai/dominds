...
在此 rtws（运行时工作区）中当前 Dominds 实现的 MCP 已实现（使用官方 MCP TypeScript SDK）：

- `.minds/mcp.yaml` 加载器，支持强制热重载。
- MCP 衍生的工具/工具集注册到现有的全局工具（集）注册表。
- 支持的传输方式：`stdio` 和 `streamable_http`（SSE 传输不支持作为独立的配置选项）。
- rtws 问题通过 WebUI（问题药丸+面板）呈现，用于 MCP 和 LLM 提供者的拒绝。
...
**rtws 级别（拒绝此重新加载尝试；保持最后已知良好的集合不变）：**
