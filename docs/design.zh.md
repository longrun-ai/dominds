...
```
主对话（根对话）
├── 任务文档引用 → tasks/feature-auth.tsk/（工作区任务文档包）
├── 提醒项（工作记忆）
├── 聊天消息（临时）
└── 子对话（树结构，扁平存储在主对话下）
    ├── 专业代理 A
...
- 所有对话引用相同的工作区任务文档（一个 `*.tsk/` 任务文档包，例如 `tasks/feature-auth.tsk/`）
...
1. **任务文档引用**：指向跟踪特定 DevOps 任务的工作区任务文档
...
- **`clear_mind`**：用于清除对话噪音和开始新轮次的函数工具
- **`change_mind`**：用于跨对话层次结构更新权威任务文档的函数工具（无轮次重置）
...
- 工作区硬性规则：
  ...
  - `.minds/**` 是保留的工作区状态（团队配置/内存/资产），所有通用文件工具都硬性拒绝；通过 `team-mgmt` 等专用工具进行管理。
