...
注：本文的“任务文档”指差遣牒（Taskdoc）。它不是个人草稿本；它是跨主线/跨智能体的任务实时协调公告板。
...
```
主对话（根对话）
├── 任务文档引用 → tasks/feature-auth.tsk/（rtws 任务文档包）
├── 提醒项（工作记忆）
├── 聊天消息（临时）
└── 子对话（树结构，扁平存储在主对话下）
    ├── 专业智能体 A
...
- 所有对话引用相同的 rtws 任务文档（一个 `*.tsk/` 任务文档包，例如 `tasks/feature-auth.tsk/`）
- 多个对话树可以引用相同的任务文档进行协作工作
- 任务文档超越单个对话持续存在并经历团队变化
- 子对话可以是具有无限嵌套深度的树结构
- 所有子对话状态扁平存储在主对话的 `subdialogs/` 目录下
- 每个子对话在引用相同任务文档的同时维护自己的工作记忆

### 记忆层

#### 对话范围记忆（每个对话）

1. **任务文档引用**：指向跟踪特定 DevOps 任务的 rtws 任务文档
   - `*.tsk/` 任务文档包（`goals.md`、`constraints.md`、`progress.md`）
   - 多个对话可以引用相同的任务文档进行协作工作
   - 任务文档在整个产品生命周期中持续存在，跨越多个对话和团队变化
   - 可以链接到其他产品文档并随项目要求演变
...
- **向上**：子对话向父级传达结果和升级
- **向下**：父级向子对话提供上下文和目标
- **横向**：通过共享任务文档和父级调解进行协调
- **时间性**：提醒项和任务文档提供跨时间的连续性
...
- **`change_mind`**：用于跨对话层次结构更新权威任务文档的函数工具（不会开启新一程对话）
...
2. **任务文档专注**：在做出自主决策或改变方向时，始终将任务文档作为主要真实来源
...
任务文档是一个封装的 `*.tsk/` 任务文档包，跟踪特定的 DevOps 任务：
...
- 多个对话树可以引用相同的任务文档
- 团队成员通过 `change_mind` 操作更新相同的任务文档进行协作
- 进度跟踪跨越对话持续存在
- 要求通过 `change_mind` 操作演变
- rtws 硬性规则：
  - `*.tsk/**` 是封装的任务文档状态，所有通用文件工具都硬性拒绝。
...
每个任务文档代表一个独立的 DevOps 任务，可以并行处理，多个对话树在保持各自对话上下文的同时就相同目标进行协作。
...
关键洞察是，AI 智能体和人类一样，从心理卫生实践中受益匪浅。定期的认知清理和专注的问题解决会话可以显著提高智能体推理的质量和效率。扪心自问（FBR）将管理大量对话上下文的挑战转化为优势，允许智能体通过中央任务文档保持整体项目连续性的同时，对复杂问题的每个方面带来新鲜、专注的注意力。
