...
我们希望有一种安全的方式让"团队管理器"智能体（通常是影子队友 `fuxi`）能够：
...
- "引导"团队管理器应该能够在无法更改产品代码、`.dialogs/` 等的情况下配置团队
...
历史上，部分指导内容位于 `dominds/` 源代码树中的传统内置"团队管理器"心智集中。该传统内置内容正在被移除。运行时的"单一事实来源"应该是 `team_mgmt_manual` 工具的输出。
...
这对于"普通"智能体来说是合理的，但它阻止了团队管理器完成其工作。

## 目标 / 非目标

**目标**

- 启用受信任的团队管理器仅管理 `.minds/` 配置面
...
- 包括完整的 `.minds/` 生命周期（创建、更新、重命名/移动、删除）。团队管理器必须能够纠正错误并从意外损坏中恢复（包括其他工具引入的损坏）
- 对 `.minds/team.yaml` 进行任何更改后，团队管理器应运行 `team_mgmt_validate_team_cfg({})` 以确保检测并暴露所有错误（并避免静默忽略损坏的成员配置）
- 路径处理应该严格：
  - 拒绝绝对路径
  - 拒绝包含 `..` 的路径
  - 拒绝规范化后解析到 `.minds/` 之外的任何路径
- 优先使用显式白名单而非" rtws 中的任何内容"
  - 对于 `team-mgmt`，该显式白名单是 `.minds/**`（包括 `.minds/memory/**`），以便团队管理器可以修复其他工具造成的意外损坏（即使 `.minds/memory/**` 已有专用的 `memory` / `team_memory` 工具供正常使用）
...
- 让团队管理器能够从"零状态"安全地创建 `.minds/team.yaml`
- 即使成员的目录允许/拒绝列表为空，也保持作用域边界
- 便于授予临时智能体仅团队管理能力而无需完整的 rtws 访问

## `team_mgmt_manual`

我们需要单一的聊天内手册工具，以便团队管理器能够可靠地自助指导，而无需阅读源代码。
...
作为从传统内置团队管理器知识文件迁移的一部分，手册必须至少涵盖以前驻留在那里的信息：
...
- 保持 `.minds/team.yaml` 的所有权严格；只有团队管理器应该能够编辑它
...
- 影子 `fuxi` 实例应该获得 `team-mgmt`（和手册工具），而不是广泛的 `ws_mod`
- 影子 `pangu` 实例应该获得广泛的工作区工具集（例如 `ws_read`、`ws_mod`、`os`），但不获得 `team-mgmt`
