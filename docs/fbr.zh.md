...
扪心自问（FBR）是 Dominds 的常用工作模式：智能体向自己发起自诉请（`!?@self`），创建一个短生命周期的支线对话，让“新靴版本的自己”围绕一个边界清晰的子问题做推理，再把结论回贴给诉请者。
...
2. **可配置的 FBR 并发**：在 `.minds/team.yaml` 增加 `fbr-effort`（默认 `1`）：
   - `0` 禁用该队友的 `!?@self` FBR
...
- 不改写通用的队友诉请分类法（详见 `dominds/docs/dialog-system.md`）；本文档仅约束 `@self` FBR。

## 定义

- **扪心自问（FBR）**：Fresh Boots Reasoning——从第一性原理出发，不依赖既有对话历史的推理方式。
...
- **不依赖诉请者线程上下文**：
  ...
  - 被诉请者不得通过工具去读文件/浏览/跑命令，或获取 Memory / 工作区状态

直觉上，可以把被诉请者理解为“相对诉请者线程是新靴的”：它不会继承诉请者累积的对话历史；但它仍可能接收一些无条件注入的基础上下文（人设/系统规则/安全规范/格式规范/只读 Memory 摘要等）。当使用 `!tellaskSession` 形式时，该支线对话还会包含自身的会话历史。

### 提示词契约（system + tool prompts）
...
- 发起 `@self` FBR 支线对话的 LLM 请求必须做到 **0 个可用工具**：
  ...
  - 不得启用 provider 支持的任何“工具调用模式 / tool choice / function calling 开关”
- 即便 provider SDK 默认会接受工具调用，运行时也必须拒绝 FBR 中模型发出的任何工具调用企图。
...
- 你**没有任何工具**，不能调用工具。
- 你不能直接访问工作区 / 文件 / 浏览器 / shell（工具调用已禁用）。
...
如果某个 provider 集成通常会注入 tool prompt（或工具 schema），那么对 `@self` FBR 必须二选一：

- 完全不注入 tool prompt，或
- 注入一个明确声明“没有工具可用/工具调用已禁用”的 tool prompt
...
两个新增字段都是**按队友（per-teammate）**配置的；也可以放在 `member_defaults` 下作为全局默认。
...
- 每条支线对话收到相同的诉请正文（以及相同的无工具 system/tool 提示约束）。
...
- 本规范刻意让 `!?@self` 与一般的瞬态支线对话行为不同：
  普通 `!?@<队友>` 的支线对话仍然是“全能力”的（可拥有工具/工具集），其行为以 `dominds/docs/dialog-system.md` 为准。
- 若你需要“同人设但可用工具”的“新靴支线对话”，应使用一个明确的队友身份并为其授予相应工具集，而不是使用 `@self`。

## 验收标准（实现检查清单）

- `!?@self` 创建的 FBR 支线对话在技术上不提供任何 tools，且 system/tool 提示明确声明“无工具 + 诉请正文为主上下文”。
- `fbr-effort` 默认 `1`；允许 `0..100`；`>100` 与非整数应报错。
- `fbr-effort: 0` 时 `!?@self` 必须清晰失败（不可静默忽略）。
- `fbr_model_params` 仅对 `@self` FBR 生效，且 schema 与 `model_params` 一致。
