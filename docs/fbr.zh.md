...
- **FBR 轮次共享窗口**：
  ...
  - 所有轮次在同一 FBR 子对话内连续推理，形成一段共享上下文历史，同时继续与主线对话历史隔离
- **不得通过工具补上下文**：
  - 不得读文件/跑命令/浏览
...
FBR 支线对话应产出一份便于诉请者整合的简明推理结果。单次 `freshBootsReasoning` 内每轮都给出一段可并置的结论，运行时会将该轮次结论按序聚合后回贴到上游。

1. **各轮结论（按轮次可索引）**：每轮不复用上轮结论，采用新切入角度。
2. **结论 / 建议**
3. **推理过程**（仅基于诉请正文）
4. **前提假设**（明确标注来源：诉请正文/会话历史）
5. **未知与缺口**（如果有）
6. **给诉请者对话的下一步**（诉请者对话可能有工具/队友，可在诉请者对话执行）
...
- 运行时必须把一条 `freshBootsReasoning({ tellaskContent: "..." })` 扩展为单一 FBR 子对话的 **N 轮顺序推理**
- 每轮接收相同的诉请正文与 FBR 限制框架；运行时只强制“本轮角度不同且不复述前轮结论”，不预设具体分析方向
- 首轮提供完整诉请正文；后续轮次仅追加该轮增量提示，不重复首轮前置说明
- 每轮必须采用与之前轮次都不同的角度，避免复用上一次推理路径
- 仅最后一轮触发回贴（单条上游回贴），但上游应接收该单一 FBR 子对话内 N 轮全部结论拼接内容，不得遗漏中间轮次关键结论
...
```yaml
member_defaults:
  # 默认每次 `freshBootsReasoning({ tellaskContent: "..." })` 在单一 FBR 子对话内串行执行 3 轮。
  fbr-effort: 3

members:
  ux:
    # 每次 `freshBootsReasoning({ tellaskContent: "..." })` 在单一 FBR 子对话内串行执行 5 轮。
    fbr-effort: 5
...
- `freshBootsReasoning({ tellaskContent: "..." })` 的 FBR 支线对话是 **特例**：无工具、正文优先、诉请受限、可串行扩展。
