...
import { assembleDriveContextMessages } from '../../main/llm/driver-v2/context';
...
async function main(): Promise<void> {
  {
    ...
    assertOrder(
      ...
      [
        ...
        'prompting:internal-priming',
      ],
      'tail insertions should happen before the last user prompt-like message',
    );
  }
  ...
  console.log('driver-v2 context-assembly-order: PASS');
}

void main().catch((err: unknown) => {
  const message = err instanceof Error ? err.message : String(err);
  console.error(`driver-v2 context-assembly-order: FAIL\n${message}`);
  process.exit(1);
});
