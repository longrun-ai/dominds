...
async function main(): Promise<void> {
  await withTempRtws(async (tmpRoot) => {
    ...
    await driveDialogStream(
      dlg,
      { content: trigger, msgId: 'driver-v2-tool-round-context-continuity', grammar: 'markdown' },
      true,
    );
  ...
  console.log('driver-v2 multi-iter-tool-round-context-continuity: PASS');
}

void main().catch((err: unknown) => {
  const message = err instanceof Error ? err.message : String(err);
  console.error(`driver-v2 multi-iter-tool-round-context-continuity: FAIL\n${message}`);
  process.exit(1);
});
