...
(() => {
  const dialogs: ApiMainDialogResponse[] = [
    makeBaseDialog({ rootId: 'r1', lastModified: 't0' }), // main row (selfId undefined)
  ...
  assert.equal(res.dialogs[0].lastModified, 't1', 'main row should bump for sideDialog activity');
...
(() => {
  ...
  assert.equal(res.changed, true);
  assert.equal(res.dialogs[0].lastModified, 't1', 'main row should bump for main dialog activity');
