...
runTest('Type B parsing - agentId with topicId', () => {
  const result = parseFromHeadline('@pangu !topic code-review');

  assertTrue(result.type === 'B', 'Should be Type B');
  assertEqual((result as TeammateCallTypeB).agentId, 'pangu', 'AgentId should be pangu');
  assertEqual(
  ...
  console.log(`Parsed: @pangu !topic code-review → Type B (${result.type})`);
});

// Test 2: Type C parsing - agentId only (no topicId)
runTest('Type C parsing - agentId only (no topicId)', () => {
  const result = parseFromHeadline('@pangu');

  assertTrue(result.type === 'C', 'Should be Type C');
  assertEqual((result as TeammateCallTypeC).agentId, 'pangu', 'AgentId should be pangu');

  console.log(`Parsed: @pangu → Type C (${result.type})`);
});
...
runTest('Type B with whitespace variations', () => {
  const testCases = [
    { input: '@pangu!topic code-review', agentId: 'pangu', topicId: 'code-review' },
    { input: '@pangu !topic code-review', agentId: 'pangu', topicId: 'code-review' },
    { input: '@pangu  !topic  code-review', agentId: 'pangu', topicId: 'code-review' },
    { input: '@pangu !topic   code-review', agentId: 'pangu', topicId: 'code-review' },
  ];
...
runTest('Type B with trailing punctuation after topic', () => {
  const result = parseFromHeadline('@pangu !topic env-check. Please confirm.');
  assertTrue(result.type === 'B', 'Should be Type B');
  assertEqual((result as TeammateCallTypeB).agentId, 'pangu', 'AgentId should be pangu');
...
runTest('Type C without topic marker', () => {
  ...
  assertTrue(!('topicId' in result), 'Type C should not have topicId');
});

// Test 8: Type discrimination
runTest('Type discrimination', () => {
  const typeBResult = parseFromHeadline('@pangu !topic issue-123');
  const typeCResult = parseFromHeadline('@pangu');
...
runTest('Complex real-world examples', () => {
  const examples = [
    ...
    { input: '@pangu', expectedType: 'C' as const },
  ];
...
runTest('Discriminated union type narrowing', () => {
  ...
  if (result.type === 'B') {
    ...
    assertEqual(result.agentId, 'pangu', 'AgentId should be pangu in Type B');
  ...
  const resultC = parseFromHeadline('@pangu');

  if (resultC.type === 'C') {
    // TypeScript should narrow to TeammateCallTypeC
    assertEqual(resultC.agentId, 'pangu', 'AgentId should be pangu in Type C');
...
runTest('Agent ID format validation', () => {
  ...
  const validAgents = ['pangu', 'reviewer', 'test_agent', 'my-agent', 'Agent123'];
...
runTest('Topic ID format validation', () => {
  const testCases = [
    { input: '@pangu !topic topic1', topicId: 'topic1' },
    { input: '@pangu !topic my-topic', topicId: 'my-topic' },
    { input: '@pangu !topic test_topic_123', topicId: 'test_topic_123' },
    { input: '@pangu !topic Task-456', topicId: 'Task-456' },
  ];
