...
export async function requireCodexPrompt(model: string): Promise<string> {
  const prompt = await loadCodexPrompt(model);
  if (prompt === null) {
...
export function requireCodexPromptSync(model: string): string {
