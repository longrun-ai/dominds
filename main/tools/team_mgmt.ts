  ...
  if (language === 'zh') {
    return (
      ...
      fmtList([
        ...
        '推荐分流：角色职责/边界/长期方法论 -> `persona/knowhow/pitfalls`；成员个人长期可复用经验与个人工作索引 -> `personal_memory`（推荐配给时同步配 `skills` 与 `resources` 工具集）；需要向全队同步的当前有效状态、关键决策、下一步与仍成立阻塞 -> Taskdoc `progress`（准实时任务公告牌）；个人/当前对话短期工作集、接续包、临时 bridge 细节 -> reminders；团队共享长期约定/不变量 -> `team_memory`。',
