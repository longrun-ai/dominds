...
function ensureMindsScopedPath(rel: string): { rel: string; abs: string } {
  ...
  const relativeFromMinds = path.relative(mindsAbs, abs);
  const isInside =
    relativeFromMinds === '' ||
    (!relativeFromMinds.startsWith('..') && !path.isAbsolute(relativeFromMinds));
  if (!isInside) {
  ...
  if (language === 'zh') {
    return (
      ...
      fmtList([
        ...
        '这些 `.minds/team/<id>/*` 文件属于团队对该角色的长期定义资产，默认应保持稳定、稀疏、慢变；它们不是该成员自己的日常经验仓库。成员在工作中积累的日常经验、近期排障线索、短中期可复用笔记，默认应优先沉淀到 `personal_memory`；凡是推荐给成员配 `personal_memory` 的场景，也默认同时推荐配 `skills` 与 `resources` 工具集，让成员能维护个人技能指引并读取已登记资源。需要向全队同步的当前有效状态、关键决策、下一步与仍成立阻塞，应写入 Taskdoc `progress` 这一准实时任务公告牌；个人/当前对话短期手头工作与临时 bridge 细节则留在 reminders。',
...
export function renderPermissionsManual(language: LanguageCode): string {
  if (language === 'zh') {
    return (
      ...
      fmtList([
        ...
        '在当前内建模型中，`.minds/team/**`、`.minds/team.yaml`、`.minds/skills/team_shared/**`、`.minds/skills/linkable/**`、`.minds/priming/**` 等团队资产目录，只有持有 `team_mgmt` 的成员才应修改。普通成员若被授予 `skills` toolset，只能通过专用工具管理自己的 `.minds/skills/individual/<member-id>/**`。',
        '`.minds/team/<id>/*` 的“角色归属对象”只表示它描述哪个角色，不构成任何额外写权限。未持有 `team_mgmt` 的成员，即使只是想更新“自己的” persona/knowhow/pitfalls，也应在诉请回复中提供建议内容，由具备权限的团队管理者代写。',
  ...
  return (
    ...
    fmtList([
      ...
      'In the current built-in model, team asset paths such as `.minds/team/**`, `.minds/team.yaml`, `.minds/skills/team_shared/**`, `.minds/skills/linkable/**`, and `.minds/priming/**` should only be modified by members who hold `team_mgmt`. Ordinary members granted the `skills` toolset may only manage their own `.minds/skills/individual/<member-id>/**` through dedicated tools.',
...
export function renderMindsManual(language: LanguageCode): string {
  if (language === 'zh') {
    return (
      ...
      fmtList([
        ...
        '推荐分流：角色职责/边界/长期方法论 -> `persona/knowhow/pitfalls`；成员个人长期可复用经验与个人工作索引 -> `personal_memory`（推荐配给时同步配 `skills` 与 `resources` 工具集）；需要向全队同步的当前有效状态、关键决策、下一步与仍成立阻塞 -> Taskdoc `progress`（准实时任务公告牌）；个人/当前对话短期手头工作、接续包、临时 bridge 细节 -> reminders；团队共享长期约定/不变量 -> `team_memory`。',
        ...
        '文件名规则（当前实现）：rtws 成员文件读取顺序为 `persona.zh.md -> persona.md`，`knowhow.zh.md -> knowhow.md`，`pitfalls.zh.md -> pitfalls.md`。旧名 `knowledge/lessons` 不再作为 fallback 接受；请直接使用 canonical 文件名。',
        '若该成员承担团队管理职责（尤其获得 `team_mgmt`），其 `persona.*.md` 必须明确写出：执行任何团队管理操作前先查看 `man({ "toolsetId": "team_mgmt" })` 的相关章节，并按手册标准做法维护 `.minds/**` 团队心智资产。',
        '语言选择（按当前实现）：优先读取 `persona.zh.md` / `knowhow.zh.md` / `pitfalls.zh.md` 这类工作语言文件，再按各自 fallback 顺序回退到无语言后缀文件；不会跨语言回退到另一种语言文件。',
  ...
  return (
    ...
    fmtList([
      ...
      'Filename rule (current implementation): rtws member files read `persona.en.md -> persona.md`, `knowhow.en.md -> knowhow.md`, and `pitfalls.en.md -> pitfalls.md`. Legacy `knowledge/lessons` names are no longer accepted as fallback; use canonical filenames directly.',
      ...
      "Language selection (current implementation): prefer work-language variants such as `persona.en.md` / `knowhow.en.md` / `pitfalls.en.md`, then fall back to each file kind's default filename. There is no cross-language fallback to another language-specific file.",
...
export function renderSkillsManual(language: LanguageCode): string {
  if (language === 'zh') {
    return (
      ...
      fmtList([
        ...
        '普通成员自管：授予 `skills` toolset 后，成员可用 `add_personal_skill` / `import_personal_skill_from_file` / `replace_personal_skill` / `move_personal_skill` / `drop_personal_skill` 管理自己的个人 skill；这些工具不会写入团队共享目录，也不能替别人写。',
      ...
      fmtList([
        ...
        '例 1：代码评审协作。skill 写“变更面梳理 -> 风险分级 -> 验证计划 -> 诉请回复格式 -> 需要升级给架构/测试的条件”；team_memory 写当前仓库的长期模块边界索引；personal_memory 写某成员常负责模块的入口路径；team.yaml 绑定谁有测试/浏览器/toolset 权限。',
  ...
  return (
    ...
    fmtList([
      ...
      'Ordinary member self-management: once a member has the `skills` toolset, it may use `add_personal_skill` / `import_personal_skill_from_file` / `replace_personal_skill` / `move_personal_skill` / `drop_personal_skill` for its own personal skills only; these tools do not write team-shared directories and cannot write for other members.',
