...
function renderMemberProperties(language: LanguageCode): string {
  ...
  if (language === 'zh') {
    return (
      ...
      fmtList([
        ...
        '`name` / `icon` / `gofor`',
        '`gofor`：该长期 agent 的职责速记卡（建议 5 行内），用于快速路由/提醒。支持 string / YAML list / YAML object；object 的 key 完全 freeform（value 必须是 string）。若你写的是 `- 标签: 内容` 这类结构化 list，虽然允许，但 `team_mgmt_validate_team_cfg({})` 会给 warning，建议改成 object；普通职责 bullet 更适合 list。对象的渲染顺序跟 YAML key 写入顺序一致（当前实现/依赖）。详细规范请写入 `.minds/team/<id>/*` 或 `.minds/team/domains/*.md` 等 Markdown 资产。',
  ...
  return (
    ...
    fmtList([
      ...
      '`name` / `icon` / `gofor`',
      '`gofor`: a short responsibility flashcard (≤ 5 lines) for a long-lived agent; use it for fast routing/reminders. It accepts string / YAML list / YAML object; object keys are fully freeform (values must be strings). If you write a structured list like `- Label: value`, it is still allowed, but `team_mgmt_validate_team_cfg({})` will warn and suggest YAML object form instead; plain responsibility bullets fit YAML lists better. Object rendering order follows the YAML key order (implementation-dependent). Put detailed specs in Markdown assets like `.minds/team/<id>/*` or `.minds/team/domains/*.md`.',
      '`provider` / `model` / `model_params`',
      '`toolsets` / `tools`（两者可同时配置；多数情况下推荐用 toolsets 做粗粒度授权，用 tools 做少量补充/收敛。具体冲突/合并规则以当前实现为准）',
  ...
  if (language === 'zh') {
    return (
      ...
      fmtList([
        ...
        '团队机制默认范式是“长期 agent”（long-lived teammates）：`members` 列表表示稳定存在、可随时被诉请的队友，并非“按需子角色/临时 sub-role”。这是产品机制，而非部署/运行偏好。\n如需切换当前由谁执行/扮演，用 CLI/TUI 的 `-m/--member <id>` 显式选择。\n`members.<id>.gofor` 用于写该长期 agent 的“职责速记卡/工作边界/交付物摘要”（建议 5 行内）：用于快速路由与提醒；更完整的规范请写入 `.minds/team/<id>/*` 或 `.minds/team/domains/*.md` 等 Markdown 资产。它支持三种形态：string（单句）、YAML list（普通 bullet）、YAML object（带标签的结构化摘要，object key 完全 freeform，value 必须是 string）。\n示例（gofor, list）：\n```yaml\nmembers:\n  qa_guard:\n    name: QA Guard\n    gofor:\n      - Own release regression checklist and pass/fail gate\n      - Maintain script-style smoke tests and how to run them\n      - Reject changes that break lint/types/tests (or request fixes)\n      - Track high-risk areas and required manual verification\n```\n示例（gofor, object；按 YAML key 顺序渲染，key freeform）：\n```yaml\nmembers:\n  coordinator:\n    name: 协调者\n    gofor:\n      Scope: 监督与协调全队，维护开发计划主线\n      Deliverables: 任务拆分方案、subagent 调度、结果集成与验收\n      Interfaces: 接收各队友的诉请；dev 实例（pnpm dev）由 @mentor 维护\n      Non-goals: 不执行 typecheck / test / build 等构建验证命令\n```\n如果你把这类结构化内容写成 `- Scope: ...` / `- Deliverables: ...` 的 YAML list，也仍然允许；但 `team_mgmt_validate_team_cfg({})` 会给 warning，建议改用 object，因为可读性更好。',
  ...
  return (
    ...
    fmtList(
      common.concat([
        ...
        'The team mechanism default is long-lived agents (long-lived teammates): `members` is a stable roster of callable teammates, not “on-demand sub-roles”. This is a product mechanism, not a deployment preference.\nTo pick who acts, use `-m/--member <id>` in CLI/TUI.\n`members.<id>.gofor` is a responsibility flashcard / scope / deliverables summary (≤ 5 lines). Use it for fast routing/reminders; put detailed specs in Markdown assets like `.minds/team/<id>/*` or `.minds/team/domains/*.md`.\nIt supports three shapes: string (single sentence), YAML list (plain bullets), and YAML object (structured labeled summary; object keys are fully freeform and values must be strings).\nExample (`gofor`, list):\n```yaml\nmembers:\n  qa_guard:\n    name: QA Guard\n    gofor:\n      - Own release regression checklist and pass/fail gate\n      - Maintain runnable smoke tests and docs\n      - Flag high-risk changes and required manual checks\n```\nExample (`gofor`, object; rendered in YAML key order, freeform keys):\n```yaml\nmembers:\n  coordinator:\n    name: Coordinator\n    gofor:\n      Scope: oversee the team and keep the delivery plan on track\n      Deliverables: task breakdown, subagent scheduling, integration, acceptance\n      Interfaces: receives teammate requests; @mentor maintains the dev instance\n      Non-goals: does not run typecheck / test / build validation commands\n```\nIf you write the same structured content as a YAML list like `- Scope: ...` / `- Deliverables: ...`, it is still accepted, but `team_mgmt_validate_team_cfg({})` will warn and suggest YAML object form for readability.',
...
export const teamMgmtValidateMcpCfgTool: FuncTool = {
  ...
  async call(dlg, _caller, _args: ToolArguments): Promise<string> {
    ...
    try {
      ...
      let fallbackInvalidToolsetManuals: ReadonlyArray<{ serverId: string; errorText: string }> =
        [];
      ...
      try {
        ...
        fallbackInvalidToolsetManuals = [...manualInfo.invalidByServerId.entries()].map(
          ([serverId, errorText]) => ({ serverId, errorText }),
        );
      } catch (err: unknown) {
        if (!(isFsErrWithCode(err) && err.code === 'ENOENT')) {
          throw err;
