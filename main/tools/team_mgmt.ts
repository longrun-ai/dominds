  ...
  if (language === 'zh') {
    return (
      ...
      fmtList([
        ...
        '这些 `.minds/team/<id>/*` 文件属于团队对该角色的长期定义资产，默认应保持稳定、稀疏、慢变；它们不是该成员自己的日常经验仓库。成员在工作中积累的日常经验、近期排障线索、短中期可复用笔记，默认应优先沉淀到 `personal_memory`；当前任务进展、临时 bridge 信息、短期待办则应留在 Taskdoc `progress` 或 reminders。',
  ...
  return (
    ...
    fmtList(
      common.concat([
        ...
        'These `.minds/team/<id>/*` files are the team-defined long-lived assets for that role, and should usually stay stable, sparse, and slow-changing. They are not the member’s everyday experience warehouse. Day-to-day lessons, recent debugging clues, and medium-term reusable personal notes should usually go to `personal_memory`; current task progress, temporary bridge notes, and short-term TODOs belong in Taskdoc `progress` or reminders.',
...
export function renderMindsManual(language: LanguageCode): string {
  if (language === 'zh') {
    return (
      ...
      fmtList([
        ...
        '推荐分流：角色职责/边界/长期方法论 -> `persona/knowhow/pitfalls`；成员个人长期可复用经验与个人工作索引 -> `personal_memory`；当前任务进展/临时 bridge/短期待办 -> Taskdoc `progress` 或 reminders；团队共享长期约定/不变量 -> `team_memory`。',
  ...
  return (
    ...
    fmtList([
      ...
      'Recommended split: role responsibilities/boundaries/long-lived methodology -> `persona/knowhow/pitfalls`; a member’s own long-lived reusable experience and working index -> `personal_memory`; current task progress, temporary bridge notes, and short-term TODOs -> Taskdoc `progress` or reminders; team-shared long-lived conventions/invariants -> `team_memory`.',
