      ...
      return language === 'zh'
        ? fmtHeader('.minds/llm.yaml') +
            fmtList([
              ...
              '当前已知示例：OpenAI Responses 包装层支持 `apiQuirks: xcode.best`（或数组里包含它）。它一方面会把供应商额外发出的 `keepalive` 流事件识别为 heartbeat，而不是当作异常事件处理；另一方面也会对该网关特有的失败模式做 provider-specific failure handling，包括“同一对话上下文连续返回 empty response”时先做少量临时重试、连续达到阈值后停止自动重试，并明确提示直接继续大概率仍无真实进展，更建议先输入一些新的指令再尝试；以及把网关返回的 HTML 版 502 Bad Gateway 错误页归类为 conservative 策略重试。最小示例：\n```yaml\nproviders:\n  my_gateway:\n    apiType: openai\n    baseUrl: https://example.invalid/v1\n    apiKeyEnvVar: MY_GATEWAY_API_KEY\n    apiQuirks: xcode.best\n    models:\n      my_model: { name: \"upstream-model-id\" }\n```',
        ...
        : fmtHeader('.minds/llm.yaml') +
            fmtList([
              ...
              'Known current example: the OpenAI Responses wrapper supports `apiQuirks: xcode.best` (or an array containing it). It not only treats vendor-emitted `keepalive` stream events as heartbeat events instead of unexpected protocol noise, but also applies provider-specific failure handling for gateway-specific failures, including repeated empty responses in the same dialog context (a few temporary retries first, then an explicit stop of automatic retries with a warning that simply continuing is still unlikely to make real progress and that fresh user instructions are a better next try) and gateway-returned HTML 502 Bad Gateway pages classified into conservative retry. Minimal example:\n```yaml\nproviders:\n  my_gateway:\n    apiType: openai\n    baseUrl: https://example.invalid/v1\n    apiKeyEnvVar: MY_GATEWAY_API_KEY\n    apiQuirks: xcode.best\n    models:\n      my_model: { name: \"upstream-model-id\" }\n```',
