    ...
    return language === 'zh'
      ...
      : fmtHeader('.minds/llm.yaml') +
          fmtList([
            ...
            '`apiQuirks` is optional under `providers.<providerKey>.apiQuirks`, with type `string|string[]`; use an array for multiple quirks, for example `apiQuirks: [xcode.best, glm-via-volcano]`. It is a provider-level transport / gateway compatibility switch for non-standard upstream API behavior. It is not a `.minds/team.yaml` member field and not part of `model_params`.',
