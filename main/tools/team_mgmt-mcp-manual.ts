...
function renderRawMcpAutomaticBaselineSection(
  ...
  lines.push(
    ...fmtList([
      ...
      language === 'zh'
        ? '说明：MCP 工具名称、description 与参数 schema 已由 server 通过 `tools/list` 自动提供，并随工具函数说明进入 LLM 上下文；手册不重复罗列工具契约，只补充整体定位、使用场景、安全边界和故障处置。'
        : 'Note: MCP tool names, descriptions, and parameter schemas are already provided by the server through `tools/list` and enter LLM context as function-tool metadata; manuals should not duplicate the tool contract, only the overall positioning, use cases, guardrails, and failure handling.',
    ])
      .trimEnd()
      .split('\n'),
  );
    ...
    ...fmtList([
      language === 'zh'
        ? entry.status === 'disabled'
          ...
          : '详见各工具函数说明（以及 WebUI 工具列表）。这些信息来自 MCP `tools/list`，不需要在 `manual` 中重复维护。'
        : entry.status === 'disabled'
          ...
          : 'See the individual function-tool descriptions (and the WebUI tools list). This information comes from MCP `tools/list` and should not be duplicated in `manual`.',
    ])
      .trimEnd()
      .split('\n'),
  );
  ...
  lines.push(
    ...fmtList([
      language === 'zh'
        ? '建议团队增强：即使该 MCP 可裸用，仍建议与人类用户确认整体定位与边界，并把简短说明写入 `servers.<serverId>.manual`；不要重复维护工具列表/参数说明。'
        : 'Recommended team enhancement: even though this MCP can be used raw, confirm the overall positioning and boundaries with the human user and write a short note into `servers.<serverId>.manual`; do not duplicate tool lists or parameter docs.',
    ])
      .trimEnd()
      .split('\n'),
  );
  ...
  const tail =
    language === 'zh'
      ? [
          ...
          '说明：MCP toolset 未配置 `manual` 不等于“不可用”，但会在 Problems 中给 warning，提醒团队补充整体定位说明。标准工具说明与参数来自 MCP `tools/list`，详见各工具函数说明；manual 只写可选团队增强说明。',
      ...
      : [
          ...
          'Important: an unconfigured MCP `manual` does NOT mean the toolset is unavailable, but Dominds reports a Problems warning to encourage a short overall positioning note. Standard tool descriptions and arguments come from MCP `tools/list`; see each function-tool description, and use manual only for optional team enhancement guidance.',
...
export async function renderMcpManual(language: LanguageCode): Promise<string> {
  ...
  if (language === 'zh') {
    return (
      ...
      fmtList([
        ...
        '团队管理者建议：配置并验证 MCP 后，优先依赖 MCP server 暴露的工具 description/参数；同时给每个 MCP server 写一段简短整体定位。只有当团队需要补充更完整的使用场景、安全边界、故障处置或协作规范时，再写 `manual.contentFile` 或更详细的 inline `content + sections`。',
  ...
  return (
    ...
    fmtList([
      ...
      'Team-manager recommendation: after MCP config is validated, rely on MCP-provided descriptions/arguments first, and still write a short overall positioning note for each MCP server. Add `manual.contentFile` or more detailed inline `content + sections` only when the team needs fuller guidance about use cases, safety boundaries, failure handling, or coordination norms.',
