...
**描述：** 路径已存在，无法使用 `add_team_memory` 创建新记忆。
...
- 如果是更新/删除操作，先使用 `add_team_memory` 创建
- 检查路径是否正确
...
### Q: clear_team_memory 会影响其他成员吗？

A: 是的，`clear_team_memory` 会删除所有共享记忆，影响所有团队成员。此操作不可恢复，请谨慎使用。
