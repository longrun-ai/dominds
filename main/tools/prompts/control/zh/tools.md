...
- 看见 Dominds 当前显示哪一个回复工具，就调用哪一个；不要自行改选别的回复工具
- 任务开头若已点名回复工具名，以那个名字为准
- `replyContent` 只放最终交付正文，不要再包一层“我现在调用了 replyXXX”
- 如果你直接写出了最终诉请回复内容但没按 Dominds 点名的回复工具发送，Dominds 可能临时提醒你改用那个工具；不要把普通文本当成正式诉请回复路径，对方可能收不到正式诉请回复
...
- 压缩/合并现有提醒项
- 把换程前需要保留的信息整理成接续包；若 Dominds 已提醒当前程吃紧/告急，也可先保留多条粗略提醒项
- 删除已写入差遣牒、无需在提醒项重复保留的内容
- 删除/改写手工提醒项里误记录的系统环境状态；这类状态以 Dominds 自动维护的提醒、面板和工具输出为准
