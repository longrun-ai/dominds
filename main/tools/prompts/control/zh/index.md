...
| [tools](./tools.md)           | 使用指导、边界说明与 reply 速查表                      |
...
提醒不用于手工复制 Dominds 系统环境会自动维护的状态，例如后台进程状态、后台进行中诉请/协作、浏览器/会话附件状态等。这些状态以 runtime 自动维护的提醒/面板/工具输出为 single source of truth；手工记录很容易过时并造成认知干扰。
