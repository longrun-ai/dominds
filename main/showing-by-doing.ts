...
async function generatePrimingNoteViaMainlineAgent(options: {
  ...
  try {
    ...
    const agentForGen = Object.create(agent) as Team.Member;
    ...
    const ctxMsgs: ChatMessage[] = [{ type: 'environment_msg', role: 'user', content: prompt }];

    const result = await llmGen.genMoreMessages(
      providerCfg,
      agentForGen,
      systemPrompt,
      [],
    ...
    const saying = result.messages
      .filter((m): m is Extract<ChatMessage, { type: 'saying_msg' }> => m.type === 'saying_msg')
      .map((m) => m.content)
      .join('\n')
      .trim();

    if (saying) return saying;
  } catch (err: unknown) {
