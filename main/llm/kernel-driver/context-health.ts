...
export const KERNEL_DRIVER_DEFAULT_CRITICAL_COUNTDOWN_GENERATIONS = 5;
export const KERNEL_DRIVER_DEFAULT_CAUTION_REMEDIATION_CADENCE_GENERATIONS = 10;
...
function getOrCreateState(dialogKey: string): ContextHealthRoundState {
  ...
  const created: ContextHealthRoundState = {};
  stateByDialogKey.set(dialogKey, created);
  return created;
}

export function resetContextHealthRoundState(dialogKey: string): void {
  stateByDialogKey.delete(dialogKey);
}
...
export function resolveCriticalCountdownRemaining(
  ...
  if (!snapshot || snapshot.kind !== 'available') {
    ...
    return KERNEL_DRIVER_DEFAULT_CRITICAL_COUNTDOWN_GENERATIONS;
  }
  ...
  if (
    ...
    state.lastSeenLevel = 'critical';
    state.criticalCountdownRemaining = KERNEL_DRIVER_DEFAULT_CRITICAL_COUNTDOWN_GENERATIONS;
  }
...
export function resolveCautionRemediationCadenceGenerations(
  ...
  if (typeof configured !== 'number' || !Number.isFinite(configured)) {
    return KERNEL_DRIVER_DEFAULT_CAUTION_REMEDIATION_CADENCE_GENERATIONS;
  }
...
export function decideKernelDriverContextHealth(args: {
  dialogKey: string;
  ...
  criticalCountdownRemaining: number;
}): KernelDriverContextHealthDecision {
