...
export function buildToolResultImageBudgetLimitDetail(args: {
  byteLength: number;
  budgetBytes: number;
}): string {
...
export function buildToolResultImageBudgetKey(args: {
  genseq: number;
  toolCallId: string;
...
export function buildToolResultImageBudgetKeyForMsg(
  msg: FuncResultMsg,
  artifact: ToolResultImageArtifact,
): string {
...
export function selectLatestToolResultImagesWithinBudget(
  context: ChatMessage[],
  budgetBytes: number,
): Set<string> {
  const candidates: Array<{ key: string; byteLength: number }> = [];
  for (const msg of context) {
    if (msg.type !== 'func_result_msg') continue;
