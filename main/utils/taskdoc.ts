...
${footerLine}
	...
	指令：不要对 \`*.tsk/\` 下的任何路径调用通用文件工具（\`read_file\`, \`overwrite_entire_file\`, \`file_range_edit\`, \`file_append\`, \`file_insert_after\`, \`file_insert_before\`, \`file_block_replace\`, \`prepare_occurrence_replace\`, \`apply_occurrence_replace\`, \`list_dir\`, \`rm_file\`, \`rm_dir\`）。差遣牒状态只能通过显式的差遣牒操作进行管理。
...
${footerLine}
	...
	Directive: Do not invoke any general file tools (\`read_file\`, \`overwrite_entire_file\`, \`file_range_edit\`, \`file_append\`, \`file_insert_after\`, \`file_insert_before\`, \`file_block_replace\`, \`prepare_occurrence_replace\`, \`apply_occurrence_replace\`, \`list_dir\`, \`rm_file\`, \`rm_dir\`) on any path under \`*.tsk/\`. Task package state is managed only through explicit Taskdoc actions.
